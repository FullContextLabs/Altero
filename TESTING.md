...
Everything below uses **`.venv/bin/altero`**, the dev checkout. A global
`~/.local/bin/altero` from `uv tool install` works the same way, but only one
install should own the LaunchAgents at a time (newest caller wins).

One-time setup: `uv sync --extra menubar`. After that there is nothing to
install by hand — `altero` is the only entry point, and the TUI and the menu bar
...
Start with nothing running, so the only engine is the one you start. The
engine lock stops a second Altero engine, but not one from a different
install sharing the same accounts.

```bash
.venv/bin/altero menubar --uninstall-service 2>/dev/null || true
launchctl bootout gui/$(id -u)/com.fullcontextlabs.altero.auto 2>/dev/null || true

# nothing should be left
launchctl list | grep -i altero || echo "no services loaded"
ls ~/Library/LaunchAgents/ | grep -i altero || echo "no plists"
```
...
```bash
cd ~/src/altero
.venv/bin/altero tui              # leave it open; in another terminal:
.venv/bin/altero service status
```
...
- `program:` — the resolved binary launchd is actually holding. This is the
  ...
  `.venv/bin/altero`, not `~/.local/bin/altero`.
- `engine:` — the lock holder.
- the plist's program ends in `auto --json --backend`.

`ls ~/.altero/.surfaces/` shows one `tui-<pid>.lock` per open
surface.

### Last one out

Close the TUI. Within ~5s (30s if the backend started less than 30s ago) the
backend should delete `~/Library/LaunchAgents/com.fullcontextlabs.altero.auto.plist` and exit 0;
...
`service inactive: com.fullcontextlabs.altero.auto` ~30s after a start with no surface open:

```bash
/usr/bin/log show --last 1h --predicate 'eventMessage CONTAINS "com.fullcontextlabs.altero.auto"' --style compact
```
...
```bash
.venv/bin/altero service logs -n 20
.venv/bin/altero service logs -f          # ctrl-c to stop
ls -l ~/.altero/snapshot.json
```

The service runs `altero auto --json`, so the raw log is JSONL; `service logs`
...
.venv/bin/altero config set autoswitch.intervalSeconds 3600
for i in 1 2 3; do stat -f '%Sm %N' ~/.altero/snapshot.json; sleep 70; done
.venv/bin/altero config set autoswitch.intervalSeconds 60
```
...
```bash
ls -lh ~/Library/Logs/com.fullcontextlabs.altero.auto.{log,err} ~/Library/Logs/com.fullcontextlabs.altero.auto.*.1 2>/dev/null
```
...
(`dd if=/dev/zero bs=1m count=9 | tr '\0' 'x' >> ~/Library/Logs/com.fullcontextlabs.altero.auto.log`)
...
```bash
.venv/bin/altero auto --once ; echo "exit=$?"
```

Expect a refusal naming the holder, and **exit 1**:

```
error: another altero engine already owns this machine's accounts:
pid NNNNN (/path/to/altero/.venv/bin/altero auto)
— not starting a second one
```

The reverse must not loop: with a hand-run `altero auto` holding the lock, a
backend started by a surface logs "another altero engine holds the engine lock;
exiting cleanly" to `~/Library/Logs/com.fullcontextlabs.altero.auto.err` and exits 0 — launchd
does not restart it (`altero service status` shows it not running, no pid
...
```bash
.venv/bin/altero service status                   # not running
.venv/bin/altero auto --once ; echo "exit=$?"     # exit 2 = no switch needed
```
...
```bash
.venv/bin/altero service status                    # engine: should read not held
```
...
| hand-run `altero auto` in a terminal | `EXTERNAL` | `Engine: pid N (…)` |
...
```bash
.venv/bin/altero                       # TUI, then open the auto screen
.venv/bin/altero menubar               # installs the menu bar agent and returns
```
...
on the new code: `altero service status` shows the new `version:` and a new pid.
`altero menubar` does the same for the menu bar agent.
...
```bash
.venv/bin/altero switch                # active account changes
.venv/bin/altero alias 1 work          # alias changes
.venv/bin/altero config set autoswitch.enabled true
.venv/bin/altero config set autoswitch.enabled false
```
...
```bash
.venv/bin/altero config get autoswitch.enabled      # true by default
```
...
Already installed at `~/Applications/Altero.app` (or `/Applications` — the
backend looks in both, and `ALTERO_WIDGET_APP` names an `.app` anywhere else)
and registered:

```bash
pluginkit -m -v | grep -i altero
```

Exactly one line, id `com.fullcontextlabs.altero.widget`. The extension id and the
widget kind changed to escape a chronod descriptor cache that kept the gallery
saying "altero"; widgets placed before that change are dead and must be removed
and re-added.

Add it from the widget gallery: right-click the desktop → Edit Widgets → search
"Altero". Small, medium and large page with ‹ ›. Extra-large is
...
```bash
~/Applications/Altero.app/Contents/MacOS/Altero --placed-widgets   # {"count": N}
```

1. Place a widget. Close every TUI and quit the menu bar.
2. After ~35s: `.venv/bin/altero service status` still shows it running, the
   plist is still in `~/Library/LaunchAgents/`, and
   `~/Library/Logs/com.fullcontextlabs.altero.auto.err` has one line
...
```bash
d=~/.altero/widget-requests; N=2
printf '{"switch":{"to":%s},"at":"%s"}' "$N" "$(date -u +%FT%TZ)" > "$d/.tmp"
mv "$d/.tmp" "$d/switch-$(($(date +%s)*1000)).json"
```

Within ~1s the file is gone, `altero status` shows Account-N, `.err` has
...
- **stale at startup**: stop the backend (close every surface, remove the
  ...
  backend (`altero service start`) → the file is deleted and the active
...
- with auto on and the target at or above the threshold, the engine does
  **not** move off it on the woken tick: the manual switch started the
  ...
  ~/.altero/autoswitch_state.json` is the switch's timestamp, the
...
### Start backend (`altero service start`)

The widget host app's "Start backend" runs `snapshot.json`'s `alteroCommand`
plus `["service", "start"]`. By hand:

```bash
jq -c .alteroCommand ~/.altero/snapshot.json   # e.g. ["/Users/me/.local/bin/altero"]
.venv/bin/altero service start; echo $?
```

1. Backend stopped: prints the `service status` summary with `running (pid …)`,
   exit 0; `launchctl print gui/$UID/com.fullcontextlabs.altero.auto` shows the plist's
   program is this build.
2. Run it again: same summary, same pid (already running this build — no
   reinstall).
3. Run the global install's `altero service start` after the dev one: the
...
does not matter. Check with `pluginkit -m -v | grep -i altero` and unregister
...
- [ ] **Start backend, from the widget.** Stop the backend (close every
      surface, remove every widget, wait for it to retire — `altero service
      status` says not running). Place a widget again; after ~3 minutes it
      draws stale and offers *Start backend*. Tap it.
      → the host app launches, runs `snapshot.json`'s `alteroCommand` plus
      `["service", "start"]`, and `altero service status` shows `running (pid
      ...
      `~/Library/Logs/com.fullcontextlabs.altero.auto.err` beyond the usual startup lines.

- [ ] **Switch from a widget tap.** With the backend running and at least two
      managed, enabled accounts, tap a non-active account row.
      → within ~1s `altero status` shows that account; `.err` has `widget:
      Switched to Account-N (email), from Account-M (requested …)`; the
      request file is gone from `~/.altero/widget-requests/`;
      `jq .lastSwitchAt ~/.altero/autoswitch_state.json` is the
...
- [ ] **Upgrade-triggered reinstall.** With the backend running, note its pid
      (`altero service status`). Bump the version (`uv tool upgrade altero`, or
      edit `__version__` and reinstall the dev build), then open a TUI.
      → `needs_install` sees the plist's `ALTERO_VERSION` differ, boots the
      job out and bootstraps afresh: a **new pid**, and `launchctl print
      gui/$UID/com.fullcontextlabs.altero.auto` shows `ALTERO_VERSION` equal to the new
      version. The TUI reports the backend as the engine owner, not itself.

- [ ] **Open at Login.** In the menu bar, toggle *Open at Login* off.
      → `~/Library/LaunchAgents/com.fullcontextlabs.altero.menubar.plist` is deleted (or
...
- [ ] **Retire and return across a logout.** Place a widget, close every
      ...
      `~/.altero/`, the plist is deleted, the process exits 0,
...
```bash
.venv/bin/altero menubar --uninstall-service   # bootout + delete the plist
