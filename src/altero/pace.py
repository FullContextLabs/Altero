...
def rolled_weekly_window(window: dict | None, now: float) -> dict | None:
    ...
    menu bar and ``altero snapshot``) reports an elapsed weekly window the same
...
def projected_exhaustion_ts(pace: PaceResult, *, fetched_at: float) -> float | None:
    ...
    JSON-only because the projection assumes a constant burn rate,
