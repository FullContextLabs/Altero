"""Credential storage layer for altero.
...
and the ``.enc``-wins backup reconciliation. Split out of
...
def _active_oauth_keychain_services() -> list[str]:
    ...
    from altero.session import keychain_service_name
...
# Account-scoped siblings altero knows about, named so the unrecognized-key
...
def shared_credential_fields(credentials: str | None) -> dict | None:
    ...
    if "claudeAiOauth" in data:
        # A sibling key altero doesn't know defaults to slot-owned (fails
        ...
        if unrecognized:
            _logger.debug(
                "Live credential has sibling keys altero does not recognize "
...
class CredentialStore:
    ...
    def _write_oauth_credentials(self, credentials: str) -> None:
        ...
        account instead of serving its memoized token until restart; it also
        keeps the file consistent for the container ``~/.claude`` sharing consumer
        rather than stranding it on stale content. Keychain-only users keep
        ...
        if self._use_keychain() and _active_store_is_the_default_secure_store():
            try:
                ...
                #; best-effort, never creates one — see the helper.
    ...
    def _refresh_stale_credentials_file(self, credentials: str) -> None:
        ...
        Rewrite-when-present / never-create. Claude Code invalidates its
    ...
    def _uses_file_backup_backend(self) -> bool:
        ...
        ~2,500 bytes). macOS uses the Keychain while it's usable and falls back
    ...
    def _retain_previous_backup(
        ...
        if unreadable:
            ...
            # decline. the base branch has no `.prev` at all, so this is no
    ...
    # is the documented /login + `altero add [--slot N]`, and these files are
    ...
    # not inherit the Keychain's failure modes (a flaky Keychain
