...
``altero run NUM|EMAIL`` launches Claude Code with ``CLAUDE_CONFIG_DIR``
...
A manifest records what altero created so removal never touches user data.
...
from altero.claude_locks import proper_lockfile
from altero.exceptions import (
    ClaudeCodeLockTimeout,
    CredentialReadError,
    SessionError,
)
from altero.fsutil import replace_with_retry
from altero.locking import FileLock
from altero.models import Platform
from altero.paths import get_default_global_config_path
from altero.printer import accent, dimmed, muted, warning
from altero.process_detection import ClaudeSession, scan_sessions
from altero.settings import atomic_write_json
...
# Records which entries in a session profile altero created (so --no-share and
# re-syncs only ever remove altero-managed links/copies, never user data).
SHARE_MANIFEST = ".altero-shared.json"
...
# profile must be re-bootstrapped on the next non-live `altero run` even if it
# still passes the local reuse check.
STALE_MARKER = ".altero-stale-credentials"

# The user-scope MCP key mirrored from the default profile's .claude.json.
MCP_KEY = "mcpServers"

# Adoption marker: this profile's mcpServers is (or was) altero-mirrored. Gates
...
MCP_MIRROR_MARKER = ".altero-mcp-mirror-v1"
...
MCP_DISPLACED_STASH = ".altero-mcp-displaced.json"
...
def is_session_stale(session_dir: Path) -> bool:
    ...
    profile marked by an older altero on this machine has a pending
...
# warning: `altero run N` is an explicit request for account N, so letting an
...
def read_session_credentials(session_dir: Path) -> str | None:
    ...
    location stays claude's job (see the module docstring on why altero never
...
def _artifacts_say_usable(session_dir: Path, email: str, org_uuid: str) -> bool:
    ...
    The probe-timeout fallback, lifted to a function
...
class SessionManager:
    ...
    def exec_default(self, claude_args: list[str]) -> NoReturn:
        ...
        Used by `altero run` (no account) when the cwd has no mapping, or its
    ...
    def _exec(self, claude_bin: str, claude_args: list[str], env: dict[str, str]) -> NoReturn:
        ...
        POSIX: ``execvpe`` replaces the altero process entirely (the lock is
    ...
    def _ensure_not_api_key(self, account_num: str, email: str) -> None:
        ...
        if self.switcher._account_kind(account_num) == "api_key":
            raise SessionError(
                ...
                "'altero run' (session mode) does not support API-key accounts yet. "
                "Use 'altero --switch-to' to make it your default login instead."
            )
    ...
    def setup_session(
        ...
        # a second `altero run` joining a live session must not invalidate
        ...
        if pre_creds and self._has_refresh_token(pre_creds):
            ...
            if outcome.error is not None and outcome.credentials:
                ...
                raise SessionError(
                    ...
                    f"re-add it: altero --add-account --slot {account_num}"
                )
        ...
        with FileLock(self.switcher.lock_file, timeout=_BOOTSTRAP_LOCK_TIMEOUT):
            ...
            # another `altero run` may have bootstrapped while we waited.
            ...
            if self._is_session_valid(session_dir, email, org_uuid):
                ...
                # peer `altero run` can bootstrap while we wait — and its
            ...
            # timeout here must not fail a launch that is fine. Only "unreachable" still stops: no local file can
            ...
            if verdict != "valid":
                ...
                raise SessionError(
                    ...
                    f"altero --add-account --slot {account_num}"
                )
    ...
    def _profile_matches_backup(
        ...
        """
        from altero import oauth as _oauth
    ...
    def _session_validity(
        ...
        try:
            ...
            # timeout established nothing. The artifact fallback is a
        ...
        for name in active_items:
            ...
            if dest.is_symlink():
                if name not in managed:
                    managed = [*managed, name]  # adopt: only altero links here
                if use_symlinks:
                    try:
    ...
    def _sync_mcp_servers(self, session_dir: Path, share: bool) -> None:
        """Mirror the default profile's user-scope ``mcpServers``.
        ...
        a session are overwritten the next time altero prepares the profile.
    ...
    def _read_mcp_source() -> dict | None:
        ...
        (ignoring CLAUDE_CONFIG_DIR — a nested `altero run` must not source
    ...
    def _write_manifest(self, manifest_path: Path, items: list[str]) -> None:
        ...
        fd, tmp = tempfile.mkstemp(
            dir=str(manifest_path.parent), prefix=".altero-shared-", suffix=".tmp"
        )
        try:
    ...
    def _remove_managed(dest: Path) -> None:
        """Remove an altero-created share entry (link or copy), never user data
