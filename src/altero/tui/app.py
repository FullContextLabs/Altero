"""The altero Textual application.
...
from altero import paths, printer, state_watch
from altero.models import AccountsSnapshot
from altero.snapshot_source import account_identity
from altero.settings import load_settings, load_ui_settings, set_setting
...
from altero.tui.data import ActionResult, SnapshotSource, format_duration, run_action
from altero.tui.modals import AddTokenModal, ConfirmModal, OutputModal, TokenForm
from altero.tui.theme import ALTERO_DARK, ALTERO_LIGHT


class AlteroApp(App):
    """Altero interactive dashboard."""

    TITLE = "Altero"
    CSS_PATH = "altero.tcss"
    ...
    def __init__(
        ...
        *,
        ...
        self._start = start  # "dashboard" | "watch" (`altero watch`)
        ...
        # State changed by anyone -- `altero switch` in another terminal, the
        # backend, the menu bar -- reaches this app within ~1s through this
        # watch (see altero.state_watch). Measurements do not: the store
    ...
    def on_mount(self) -> None:
        self.register_theme(ALTERO_DARK)
        self.register_theme(ALTERO_LIGHT)
        resolved = self._resolved_theme()
        # We own the theme; $TEXTUAL_THEME is intentionally not honoured.
        self.theme = f"altero-{resolved}"
    ...
    def _watch_tick(self) -> None:
        ...
        if state_watch.SETTINGS in changed:
            ...
            # follow an `altero config set` or a menu-bar change made since.
            try:
