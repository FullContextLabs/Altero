...
matter which process made the change -- ``altero switch`` in a terminal, the
...
altero reflects and comparing *that* makes a no-op rewrite cost one stat and
...
``mappings.json`` (``altero map``) is deliberately not watched: neither the TUI
...
def _active_identity(path: Path):
    """The only part of Claude Code's config that altero reflects."""
