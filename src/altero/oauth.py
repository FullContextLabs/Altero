...
from altero.printer import warning as print_warning
...
def _log_usage_failure(
    ...
    (failures swallowed at DEBUG are undiagnosable); the full
...
def _persist(
    ...
    try:
        ...
        _logger.warning(
            ...
            "with invalid_grant, re-run `altero --add-account` after logging in.",
            account_num,
            email,
            e,
        )
        # stderr, not stdout: this runs inside ``altero list --json`` and the
        ...
        print_warning(
            ...
            f"If the next refresh fails, re-run `altero --add-account` after logging in.",
            file=sys.stderr,
        )
