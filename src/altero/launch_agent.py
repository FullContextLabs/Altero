"""Run an altero subcommand as a launchd LaunchAgent, not a foreground process.

``altero menubar`` and ``altero auto`` both block the terminal that started
...
``altero upgrade``) rebuilds the tool's virtualenv — ``sys.executable`` points
...
# agent runs the foreground app (plain `altero menubar` installs the agent and
...
# lets it retire once no surface is open; a hand-run `altero auto` never does.
...
def _require_macos() -> None:
    if sys.platform != "darwin":
        raise ClaudeSwitchError(
            "Background services (menu bar, backend) are only available on macOS."
        )
...
def service_target(label: str = LABEL, uid: int | None = None) -> str:
    """launchd service target, e.g. ``gui/501/com.fullcontextlabs.altero.menubar``."""
...
def resolve_program() -> list[str]:
    ...
    install` puts a symlink at ``~/.local/bin/altero`` pointing into the tool's
    ...
    if candidate is not None:
        ...
        if absolute.name == "altero" and absolute.is_file():
            return [str(absolute)]

    which = shutil.which("altero")
    if which:
...
ENGINE_OTHER = "other"  # a hand-run `altero auto`, or another surface
...
def engine_owner(backup_dir: Path) -> tuple[str, str]:
    ...
    question from "is anything ticking": a hand-run ``altero auto``, or a
...
def _plist_version(target_plist: Path) -> str | None:
    """The altero version that wrote the plist, or None (pre-dates the tag)."""
    env = _read_plist(target_plist).get("EnvironmentVariables")
    version = env.get("ALTERO_VERSION") if isinstance(env, dict) else None
...
def set_open_at_login(enabled: bool, home: Path | None = None) -> None:
    ...
    CLI's ``altero menubar --uninstall-service`` is the stop-it-now path.
    """
    _require_macos()
...
def open_surface(backup_dir: Path, kind: str | None, home: Path | None = None):
    ...
    None ensures without registering, for ``altero menubar``, which only
    ...
    if not lifecycle.acquire():
        return None, False, "another altero is starting or stopping the backend"
    try:
        registration = (
...
# ALTERO_WIDGET_APP (full path to the .app) overrides both, for a copy kept
# anywhere else.
WIDGET_HOST_APP = Path("Applications") / "Altero.app"
SYSTEM_APPLICATIONS = Path("/Applications")
WIDGET_APP_ENV = "ALTERO_WIDGET_APP"
...
def widget_host_candidates(home: Path | None = None) -> list[Path]:
    ...
    return [app / "Contents" / "MacOS" / "Altero" for app in apps]
...
class PlacedWidgets:
    """How many altero widgets are placed; a positive answer is cached for a
    few minutes.
