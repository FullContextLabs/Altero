"""One-time, run-once data migrations for altero.

A small, boring home for compatibility migrations so they don't pollute the
core switch/read/write flow in :mod:`altero.switcher`. Each migration:
...
- raises :class:`~altero.exceptions.MigrationIncomplete` (or any other
...
from altero.exceptions import MigrationIncomplete
from altero.fsutil import replace_with_retry
from altero.models import Platform, get_timestamp
from altero.switcher import KEYRING_SERVICE, SECURITY_SERVICE
...
def migrate_windows_keyring_to_files(switcher: "ClaudeAccountSwitcher") -> bool:
   ...
   . This relocates any pre-existing keyring entries so upgrading users
    ...
    if migrated:
        print(
            f"altero: migrated {migrated} Windows credential(s) from "
...
def migrate_macos_keyring_to_security(switcher: "ClaudeAccountSwitcher") -> bool:
    ...
    :mod:`altero.macos_keychain`) instead of the third-party ``keyring``
    ...
    if migrated:
        print(
            f"altero: migrated {migrated} macOS credential(s) from the keyring "
