...
from altero import oauth, pace
from altero.claude_locks import claude_config_lock, claude_credentials_lock
...
from altero.credentials import (  # noqa: F401  (constants re-exported for migrations/tests)
...
from altero.fsutil import read_text_with_retry
from altero.locking import FileLock
from altero.logging_config import setup_logging
...
from altero.process_detection import get_running_instances
from altero import poll_policy
from altero.settings import load_settings, parse_model_names, settings_path
from altero.usage_store import (
    FetchRecord,
    UsageEntry,
    UsageStore,
    with_sentinel,
...
def _format_usage_lines(usage: dict, fetched_at: float | None = None) -> list[str]:
    ...
    for label, w in (("5h", usage.get("five_hour")), ("7d", usage.get("seven_day"))):
        if w:
            # Pace only applies to the weekly (7d) window, never 5h.
...
ERROR_NOTES = {
    ...
    "invalid_client": (
        "altero's OAuth client was rejected — systemic, not this account"
    ),
    "consume-busy": (
        "another altero surface holds the slot — retries next pass"
    ),
    "stash-unreadable": (
        "this slot's stashed successor is unreadable — unlock the keychain "
        "or fix the file, then retry; `altero unclaimed` inspects it"
    ),
}
...
SENTINEL_NOTES = {
    ...
    USAGE_RELOGIN_REQUIRED: "re-login needed — refresh token dead; log in with Claude Code, then run: altero add",
}
...
class ClaudeAccountSwitcher:
    ...
    def __init__(self, debug: bool = False):
        ...
        from altero.migrations import run_migrations
    ...
    def _read_json(self, path: Path, *, strict: bool = False) -> dict | None:
        ...
        the previous code's import path DELIBERATELY replaces a malformed config it is
    ...
    def _salvage_unreadable(
        ...
        world-readable in a file altero created. Copied without metadata and
    ...
    def _refuse_degraded_capture(self) -> str | None:
        ...
        with ``degraded`` discarded. So a locked-keychain ``altero add``
    ...
    def _read_capture_credentials(self) -> str | None:
        ...
        Read-only. altero does not write claude's hashed keychain entry — see
        the ``session`` module docstring for why.
        """
        from altero.session import read_config_dir_credentials
    ...
    def _prepare_credentials_for_activation(
        ...
        altero does not recognize — must not leak across an account switch.
    ...
    def _post_backup_write(self, account_num: str, email: str) -> None:
        ...
        material so the next `altero run` re-bootstraps from this fresh backup
    ...
    def _write_account_credentials(
        ...
        healthy slot, and ``altero run`` prints "Could not refresh the token").
    ...
    # -- public accessors for session mode (altero.session) ---------
    ...
    def set_alias(self, identifier: str, alias: str) -> tuple[str, str]:
        ...
        typo'd alias can be corrected with ``altero alias <old> <new>`` as
    ...
    def unset_alias(self, identifier: str) -> str:
        ...
        alias succeeds silently (no error), matching ``altero config unset``'s
    ...
    def swap_accounts(self, first: str, second: str) -> tuple[str, str]:
        ...
        ``sequence`` (kept sorted, so rotation and ``altero list`` order
                ...
                for kind, content in (("creds", creds), ("config", config)):
                    ...
                    if path.exists():
                        raise ConfigError(
                            ...
                            f"accounts still work (`altero list`), then delete "
    ...
    def slot_for_directory(self, directory: str | Path) -> tuple[str | None, str | None]:
        """Resolve a directory to its mapped account slot, for `altero run`.
    ...
    def list_mappings(self) -> None:
        """Print all directory → account mappings (for `altero map`)."""
        from altero.mappings import MappingStore

        mappings = MappingStore(self.backup_dir).all()
        if not mappings:
            print(dimmed("No directory mappings yet."))
            print(muted("Map one with: altero map <NUM|EMAIL> [PATH]"))
            return
        seq = self._get_sequence_data_migrated() or {}
    ...
    def switchable_account_numbers(self) -> list[str]:
        ...
        disabled (``altero disable``). Disabled slots stay managed and remain
        valid explicit ``altero switch <num|email>`` targets — they are only
    ...
    def set_account_disabled(self, identifier: str, disabled: bool) -> None:
        ...
        bare ``altero switch`` rotation, and the ``best`` / ``next-available``
        strategies all skip disabled slots. The account stays managed and is
        still a valid explicit ``altero switch <num|email>`` target, so you can
    ...
    def current_account_number(self) -> str | None:
        ...
        overwrite a login altero doesn't own (``_perform_switch`` would take
    ...
    def live_session_pids_for(self, account_num: str, email: str) -> list[int]:
        """Public wrapper: PIDs of live ``altero run`` sessions for a slot."""
    ...
    def consume_backup_grant(
        ...
        *consume lock*, in this same order, and its own comment at that site
        ...
        # CLAUDE_SECURESTORAGE_CONFIG_DIR for its credential store. altero
        # mirrors that resolution on the CAPTURE path (
        ...
        if os.environ.get("CLAUDE_SECURESTORAGE_CONFIG_DIR"):
            self._logger.warning(
                "CLAUDE_SECURESTORAGE_CONFIG_DIR is set; altero mirrors it "
    ...
    def _consume_backup_grant_locked(
        ...
        try:
            with FileLock(self.lock_file):
                ...
                # the profile (a live claude rotates its own tokens) and the profile identity — org included: two slots
        ...
        try:
            ...
            try:
                ...
                self._logger.error(
                    ...
                    "storage failure, then re-login and `altero add` if the "
    ...
    def _retire_stash_entry(self, entry_id: str, account_num: str) -> None:
        ...
        one stale row, which the next pass retries and `altero unclaimed
        ...
        try:
            ...
            self._logger.warning(
                ...
                "the next pass (`altero unclaimed --purge` drops it by hand).",
    ...
    def _adopt_stashed_successor(
        ...
        if manifest_verdict == "unreadable" or (
            ...
            # globs the entry files, so `altero unclaimed` lists the orphans by
            ...
            raise CredentialReadError(
                ...
                "successor may already have superseded (`altero unclaimed` "
        ...
        for entry_id, meta in manifest.items():
            ...
            if meta.get("consumedFp") != cur_fp:
                if meta.get("reason") == "consume-gate-cas-conflict":
                    ...
                    # operator's call (`altero unclaimed --purge`).
                    self._retire_stash_entry(entry_id, account_num)
                    self._logger.info(
            ...
            # dropped with `altero unclaimed --purge`.
            self._retire_stash_entry(entry_id, account_num)
            self._logger.info(
    ...
    def list_unclaimed_credentials(self) -> dict[str, dict]:
        ...
        documented ``/login`` + ``altero add [--slot N]``.
        """
    ...
    def _live_session_pids(self, account_num: str, email: str) -> list[int]:
        ...
        """
        from altero.session import scan_live_sessions
    ...
    def _invalidate_session_credentials(self, account_num: str, email: str) -> None:
        ...
        The next `altero run` fails the reuse check and re-bootstraps from
    ...
    def _session_profile_ahead(
        ...
        moved under it while it was live, and altero has already decided it
    ...
    def _adopt_session_credential(
        ...
        family and owns it. Decided and written under altero's own lock, since
        ...
        """
        from altero.session import profile_is_quiescent
    ...
    def _reject_live_api_key_capture(self, creds: str) -> None:
        ...
        if looks_like_api_key(creds):
            raise ValidationError(
                ...
                "'altero --add-token sk-ant-api...' instead of --add-account."
            )
    ...
    def _resolve_account_identifier(self, identifier: str) -> str | None:
        ...
        raise ConfigError(
            ...
            f"Use account number instead (e.g., altero --switch-to 1)."
        )
    ...
    def _fetch_active_usage(
        ...
        - **Provenance**: a live credential is only CONSUMED
        ...
        # pass, so a pass that started before an `altero switch` routes slot N
            ...
            if outcome.error != "http-401":
                if outcome.usage is not None:
                    ...
                    if self._probe_verdicts and self._probe_verdicts.get(
                        ...
                        # decisions (the credential-poisoning mis-keying shape). The sentinel
        ...
        # CC refreshes on its own next use, exactly the pre-guard shape.
        ...
        # mirrors it; this one does not) — the copy about to be
        ...
        try:
            # Lock order matches the switch path (switch_to): altero's own
            ...
            with (
                ...
                # Under-lock TOCTOU guards. An `altero switch` or `/login`
                ...
                if (
                    ...
                    if live_oauth is not None and (
                        ...
                        #   CC's next use (pre-guard behavior for this shape
                    ...
                    if (
                        ...
                        # (contended by `altero switch` with a 10s acquire
                        ...
                        if result.error in (
                            ...
                            # next pass consumes the newer lineage. (Discipline: local reads only, no network,
            ...
            # or another altero operation holding the account FileLock. Either
    ...
    def _read_only_fetch(
        ...
        """A fetch with a live session's credential, which altero must never
    ...
    def _slot_token_dead(self, num: str, email: str) -> bool:
        ...
        reachable from a caller that has only a slot number — `altero import`'s
    ...
    def _select_best_switchable(
        ...
        can't be proven beneficial, it stays put; bare ``altero --switch``
    ...
    def _duplicate_account_warnings(
        ...
        was overwritten with another's (the credential-poisoning end state) or the same
    ...
    def _lockstep_usage_warnings(
        ...
        state a pre-guard switch could produce) carry different
    ...
    def list_accounts(
        ...
        # here: users can't act on them (recovery is always /login + altero
    ...
    def _first_run_setup(self) -> None:
        ...
        if response.lower() == "n":
            print(dimmed("Setup cancelled. You can run 'altero --add-account' later."))
            return
    ...
    def switch(
        ...
        better, it stays put (run a plain ``altero --switch`` to rotate anyway).
        ...
        # (e.g. right after altero --import). Activate the recorded
            ...
            if target_disabled or not self._account_is_switchable(target):
                if target_disabled:
                    ...
                    console_reason = (
                        ...
                        f"altero --add-account --slot {target})"
                    )
                if json_output:
                ...
                if not fallback:
                    ...
                    raise ConfigError(
                        ...
                        "Re-add a slot with: altero --add-account --slot <number>"
                    )
                target = fallback
        ...
        if not self._account_exists(current_email, current_org_uuid):
            ...
            if json_output:
                ...
                return self._switch_noop(
                    ...
                    from_ref=ref,
                    to_ref=ref,
                    message="Active account is not managed; run altero --add-account",
                )
        ...
        # worse or unverifiable account). Bare `altero --switch` rotates anyway.
        if strategy == "best":
            ...
            if note == "current-unavailable":
                ...
                print(dimmed(
                    f"Current account usage is unavailable — staying on "
                    f"Account-{current_num}. Run altero --switch to rotate."
                ))
                return None
            ...
            if not self._account_is_switchable(candidate):
                if json_output:
                    ...
                    print(
                        ...
                        f"altero --add-account --slot {candidate})"
                    )
                continue
            if strategy == "next-available":
        ...
        if next_account is None:
            ...
            print(dimmed(
                ...
                "Re-add a skipped slot with: altero --add-account --slot <number>"
            ))
            return None
        ...
        # the live credentials (the self-switch hazard, on the strategy path).
    ...
    def switch_to(
        ...
        # Short-circuit a no-op before mutating. A self-switch
        ...
        # from the stored backup. Provenance-aware: the no-op is
                ...
                if cur_slot == target_account and action != "reconcile":
                    ...
                    if not json_output:
                        ...
                        print(dimmed(
                            ...
                            f"altero --switch-to {target_account} --force"
                        ))
                        return None
                    return self._switch_noop(
                        strategy="direct",
    ...
    def _slot_supersedes_live(backup: str | None, live: str) -> bool:
        ...
        ``altero add`` while a stale credential was still sitting in the live
        file.
    ...
    def _classify_outgoing_credential(
        ...
        - ``"foreign"``        — uuid-positively resolved to *another* managed
          ...
          here would destroy this slot's only refresh token (credential
        ...
        - ``"foreign-newer"``  — ``"foreign"``, plus the live access token
          ...
          generation of that slot's lineage, unless the slot holds a newer
          login (``"own-superseded"``'s test). Preserved in a safety copy *and*
        ...
        if oauth.is_newer_generation(
            original_creds, foreign_backup
        ) and not self._slot_supersedes_live(foreign_backup, original_creds):
            # A later access-token expiry orders rotations inside one login
            # family only; a slot re-logged since (a newer family by
            # refreshTokenExpiresAt, #302) must not be reverted to the older
            # one just because that family refreshed more recently.
    ...
    def _stash_live_credential(
        ...
        identifying what wrote the credential (the poisoning writer is unidentified).
        """
        ...
        self._logger.warning(
            ...
            "(credentials mtime %s). Something outside altero rewrote the live "
    ...
    def _read_target_credentials(self, account_num: str, email: str) -> str:
        ...
        every ordinary `altero switch` to "Re-add with: altero --add-account",
        ...
        raise SwitchError(
            ...
            f"Re-add with: altero --add-account --slot {account_num}"
        )

    def _refuse_session_shell(self) -> None:
        """Refuse live-store mutation from inside a ``altero run`` shell.
        ...
        try:
            ...
            return
        raise SwitchError(
            "This shell is inside an altero run session profile "
    ...
    def _perform_switch(
        ...
        if manual and op["from"] != op["to"]:
            from altero.autoswitch import record_manual_switch
    ...
    def _perform_switch_locked(
        ...
        # Beyond altero's own lock, hold Claude Code's advisory locks for the
        ...
        with FileLock(self.lock_file), claude_credentials_lock(), claude_config_lock():
            ...
            # (e.g. right after import), altero has no tracked active
            ...
            if force_activate or current_identity is None or current_account is None:
                ...
                # unknown to altero); a numbered ref when --force ran with a
                # managed live login.
                ...
                if not target_config:
                    raise SwitchError(
                        ...
                        f"Re-add with: altero --add-account --slot {target_account}"
                    )
                try:
                ...
                # below — and the rollback, should activation fail
                ...
                # Invariant II: this path skips the backup step,
                ...
                try:
                    ...
                    # The previous code REPLACES a malformed config here on purpose
                    ...
                    # `_get_current_account()` None. So keep the previous code's
            ...
            try:
                ...
                # owns the live bytes (an external write here
                ...
                if kind in ("foreign", "foreign-newer", "alien", "known-foreign"):
                    ...
                    if kind == "foreign-newer":
                        ...
                        msg = (
                            ...
                            "in as it and run: altero add --slot "
                            f"{foreign_slot}"
                        )
                        ...
                        msg = (
                            ...
                            "account, log in as it and run: altero add"
                        )
                    if emit_output:
                        warning(msg)
                    else:
                    ...
                    # Untouched since altero wrote it — the slot already holds
                    # these bytes. Refresh only the config backup. (Rare since
                    # Activation composes live shared MCP state into the
        ...
        if emit_output:
            ...
            try:
                ...
                print(dimmed("  (usage display unavailable — run `altero --list` to retry)"))
            print()
            self._print_switch_followup()
            print()
        self._replan_new_active(
    ...
    def purge(self) -> None:
        """Remove all traces of altero from the system.
        ...
        - The data directory (XDG path on Linux/WSL, ~/.altero elsewhere)
        """
        self._refuse_session_shell()
        ...
        warning("This will remove ALL altero data from your system:")
        ...
        if session_dirs:
            from altero.session import delete_macos_keychain_entry
        ...
        if removed_items:
            ...
            print(f"\n{dimmed('No altero data found to remove.')}")
