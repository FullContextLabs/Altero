"""``altero snapshot`` — the whole display state as one JSON document.
...
* ``usage.fiveHour.history`` — 24h of 5h-window samples from the backend's
  ...
  (the engine's file and ``altero snapshot`` both are).
* top-level ``autoswitch`` — engine state (enabled, effective threshold, next
  ...
  engine-published file carries it; ``altero snapshot`` omits the key.
* top-level ``alteroCommand`` — the argv prefix that runs this altero
  (``launch_agent.resolve_program()``, what the backend plist runs), so the
  widget's host app can run ``alteroCommand + ["service", "start"]``. Engine
...
def default_snapshot_path() -> Path:
    """Where ``altero auto`` publishes the snapshot for other processes.
...
def snapshot_payload(
    ...
    *,
    ...
    altero_command: list[str] | None = None,
) -> dict:
    ...
    has none). ``autoswitch`` is the engine's block and ``altero_command``
    ...
    if altero_command is not None:
        payload["alteroCommand"] = list(altero_command)
    return payload
...
def write_snapshot(path: Path, payload: dict) -> None:
    ...
    **Writes THROUGH a symlink, never over it**, exactly as
    ``settings.atomic_write_json`` does and for the same reason: a
