...
One versioned JSON file for user-tunable altero preferences, written
...
class AutoSwitchSettings:
    """Policy knobs for the auto-switch engine (``altero auto``).
    ...
    # what altero auto is for. Only an unset value falls back to this; an
...
class SettingSpec:
    ...
    (`_clamped`) and the strict validation in `altero config set`
...
SETTING_SPECS: dict[str, SettingSpec] = {
    ...
    for spec in (
        ...
        SettingSpec(
            ...
            help="Poll interval for the altero auto loop, in seconds",
        ),
        SettingSpec(
    ...
    for spec in SETTING_SPECS.values():
        ...
        if spec.kind in ("float", "int"):
            ...
            if isinstance(value, str):
                ...
                # quoted boolean is read by the same words `altero config set`
...
def parse_setting_value(spec: SettingSpec, raw_value: str):
    """Strictly parse a CLI-provided string for `altero config set`.
    ...
    not by silently degraded behavior at `altero auto` time.
    """
    if spec.kind == "bool":
    ...
    if spec.kind == "string":
        ...
        if not value:
            raise ConfigError(
                ...
                f"'altero config unset {spec.dotted}' to clear it"
            )
        return value
    try:
...
def set_setting(backup_root: Path, dotted_key: str, raw_value: str):
    """Validate and persist one key for `altero config set`; returns the value.
...
def effective_settings(backup_root: Path) -> list[tuple[SettingSpec, object, bool]]:
    ...
    to the default still counts — so `altero config`'s "(default)" marker
...
def atomic_write_json(path: Path, data: dict) -> None:
    ...
    **Writes THROUGH a symlink, never over it.** A rename swaps a directory
    ...
    it. Same shape as the fix to ``session.py``'s own writer;
    ...
    - The 0700 hardening stays on the directory altero owns. Applying it to
