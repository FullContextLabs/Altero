...
``<backup>/widget-requests/`` and nothing else, so it cannot run ``altero`` or
...
* ``autoswitch-<epochMillis>.json`` — ``{"autoswitch": {"enabled": bool},
  ...
  ``altero config set``, while fresh (``AUTOSWITCH_MAX_AGE_S``).
* ``switch-<epochMillis>.json`` — ``{"switch": {"to": int}, "at": ...}``. The
  newest is applied through ``switch_to``, the path behind ``altero switch
...
def apply_switch_request(
    ...
    of its own: the switch is ``switch_to``, exactly as ``altero switch <N>``
    ...
    try:
        if switcher.is_account_disabled(str(to)):
            # `altero switch N` accepts a disabled slot; the widget does not —
