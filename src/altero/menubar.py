"""macOS menu bar app for altero (``altero --menubar``).

A thin GUI shell over ``ClaudeAccountSwitcher`` and the core auto-switch engine
(``altero.autoswitch``) — it never re-implements account, usage, or
...
``AutoSwitchEngine`` the CLI's ``altero auto`` drives, sharing
...
class MenuBarSettings:
    ...
    ``altero.settings`` (the ``autoswitch.*`` keys), so the CLI, the
...
# The weekly roll-forward lives in ``pace`` so the menu bar and ``altero
...
def usage_summary(
    ...
    window that's meaningfully ahead of pace, never the 5h one.
    """
...
def engine_owner_label(state: str, detail: str) -> str:
    ...
    hand-run ``altero auto``, another surface). Claiming the first when it is
...
def _adapt_snapshot(snap) -> dict:
    ...
    measurement's fetch time, used only for the pace marker.
    """
    accounts = []
    active_email = None
    active_usage = None
...
def framework_build_warning(
    ...
    if install_method is None:
        from altero.update_check import _detect_install_method

        install_method = _detect_install_method()

    if install_method == "uv":
        remedy = (
            "  uv tool install --managed-python --force 'altero[menubar]'"
        )
    elif install_method == "pipx":
        remedy = (
            "  Reinstall against a non-framework interpreter, e.g. one from "
            "`uv python install 3.13`:\n"
            "  pipx install --force --python <that python> 'altero[menubar]'"
        )
    else:
        remedy = (
...
def run(switcher) -> int:
    """Entry point for ``altero menubar --foreground`` — what the LaunchAgent
    ...
    try:
        ...
        # guard around ``from altero.menubar import run`` can never see a
        ...
        raise ClaudeSwitchError(
            ...
            "Install with: pip install 'altero[menubar]'"
        ) from e
    ...
    from altero.autoswitch import AutoSwitchEngine, BackendEventLog
    from altero.settings import AutoSwitchSettings, load_settings, set_setting
    ...
    class MenuBarApp(rumps.App):
        def __init__(self):
            ...
            # reaches the menu within ~1s (see altero.state_watch).
            ...
            # Auto-switch engine (the same one `altero auto` runs), hosted in a
        ...
        def _detect_state_change(self):
            ...
            # `altero add` in a terminal, the backend, the TUI -- within ~1s.
        ...
        def _refresh_owner(self):
            ...
            claiming to run auto-switching while a hand-run ``altero auto``
        ...
        def _start_engine(self):
            ...
            try:
                ...
                rumps.notification("Altero", "Auto-switch failed to start", str(e))
                return
            self._engine = engine
        ...
        def _drain_engine_events(self):
            ...
            for ev in events:
                if ev.kind == "switch" and not getattr(ev, "dry_run", False):
                    rumps.notification("Altero", "Auto-switched account", ev.human())
                    self.refresh_async()  # reflect the switch promptly
                elif ev.kind == "account-quarantined":
                    rumps.notification("Altero", "Account quarantined", ev.human())
                elif ev.kind == "all-exhausted":
                    rumps.notification("Altero", "All accounts exhausted", ev.human())
                    ...
                    rumps.notification("Altero", "Configuration warning", ev.human())
            ...
            try:
                ...
                rumps.alert(
                    ...
                    message="Couldn't read the active credential. If the menu bar is running "
                            ...
                            "quit and relaunch it from a Terminal with: altero --menubar",
                )
                return
            except ClaudeSwitchError as e:
                rumps.alert(title="Altero", message=str(e))
        ...
        def on_toggle_autoswitch(self, _sender):
            ...
            try:
                ...
                rumps.alert(title="Altero", message=f"Couldn't set auto-switch: {e}")
                return
            if enabled:
                self._start_engine()
            else:
        ...
        def _make_threshold(self, pct):
            def cb(_sender):
                try:
                    ...
                    rumps.alert(title="Altero", message=f"Couldn't set threshold: {e}")
                    return
