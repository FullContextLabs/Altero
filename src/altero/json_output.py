...
# account is quarantined from fetching until a re-login (``altero login`` / ``add``)
...
def _pace_fields(entry: dict, fetched_at: float | None) -> dict:
    """Weekly-window pace fields: additive, JSON-only.

    Emitted only when pace is computable and not suppressed (see
    ``altero.pace.compute_pace``). ``projectedExhaustionAt`` is a linear
...
def usage_to_json(usage: dict, fetched_at: float | None = None) -> dict:
    ...
    windows (``seven_day``, ``scoped``) only — never ``five_hour``.
    """
    out: dict = {}
...
def usage_fields(
    ...
    is forwarded to ``usage_to_json`` for the weekly pace fields.
    """
