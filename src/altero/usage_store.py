...
entries) and ``altero auto`` (scheduled polling), so each learns from the
other's fetches.
...
# server's own deadline either way. Cumulative polling from altero's own
...
# ``altero.log`` lines "Usage fetch failed for account N: http-429,
...
class UsageEntry:
    ...
    def token_dead(
        ...
        heals it, mirroring the identity-not-slot rule). A row struck
...
def _failure_backoff_s(
    ...
    *,
    ...
    # the base branch ... the blind window was always 0"). That is false,
    ...
    # backed by a byte-identical table against a real the base branch
    ...
    # byte-identical to the previous code, whose non-429 cap is also 3600. The arm
    # this PR DOES change is the 429 arm (RETRY_AFTER_FLOOR_CAP_S: the previous code
    ...
    #
    #  age@fail   PR park  PR blind   base park      base blind
    #         0      4500         0            3600               0
    #      2701      4500         1            3600               0   <- +1s
    #      3600      4500       900            3600               0   <- +900s
    #      5000      4500      2300            3600            1400   <- +900s
    #
    # Onset moves from age 3600 (base) to age 2700 (this PR) — 900s
    # earlier — and every age past onset is a flat +900s worse than
    # the previous code. This is reachable with no contrived staleness: after one 429
    ...
    # worse than the previous code, not merely pre-existing.
    #
