"""Command-line interface for Altero."""
...
from altero import __version__, paths, printer
from altero.exceptions import ClaudeSwitchError
from altero.json_output import error_envelope
...
from altero.settings import load_ui_settings
...
def _prog_name() -> str:
    ...
    ``python.exe C:\\Users\\me\\.local\\bin\\altero``). We strip that down to the
    bare command the user typed (``altero``), falling back to
    ``altero`` for ``python -m altero`` and odd launchers.
    """
...
# users type `altero list`, `altero status`, `altero add`, etc. instead of `--list`
...
def _translate_subcommand(argv: list[str]) -> list[str]:
    ...
    exactly as before (e.g. ``altero switch --strategy best``, ``altero list --json``).
    """
    if not argv:
        return argv
...
def _run_command(argv: list[str]) -> None:
    """Handle `altero run NUM|EMAIL [--no-share] [-- <claude args>]`.
    ...
    first argument (`altero --debug run 2` is not supported; use
    `altero run 2 --debug`).
...
Examples:
  altero run 2
  altero run user@example.com
  altero run 2 --no-share
  altero run 2 --share-history
  altero run 2 --require-session
  altero run 2 -- --resume
        """,
    )
    parser.add_argument(
        ...
        "directory's mapping (see `altero map`).",
    )
    parser.add_argument(
        "--no-share",
        action="store_true",
    ...
    try:
        ...
        from altero.session import SessionManager
...
def _map_command(argv: list[str]) -> None:
    """Handle `altero map [NUM|EMAIL] [PATH]`.
    ...
    parser = argparse.ArgumentParser(
        prog="altero map",
        description=(
            "Map a stored account to a directory so `altero run` (with no "
        ...
        epilog="""
Examples:
  altero map 2 ~/work/client-app
  altero map user@example.com          # map the current directory
  altero map                           # list all mappings
        """,
    )
    parser.add_argument(
        "account",
    ...
    try:
        ...
        from altero.mappings import MappingStore, normalize_path
...
def _unmap_command(argv: list[str]) -> None:
    """Handle `altero unmap [PATH]` — remove a directory→account mapping."""
    parser = argparse.ArgumentParser(
        prog="altero unmap",
...
def _unclaimed_command(argv: list[str]) -> None:
    """Handle `altero unclaimed [--purge ID]` — inspect or drop a stash row.
    ...
    parser = argparse.ArgumentParser(
        ...
        description=(
            ...
            "Purging deletes the bytes — recovery is /login + `altero add`."
        ),
    )
    parser.add_argument(
        "--purge",
...
def _snapshot_command(argv: list[str]) -> None:
    """Handle `altero snapshot [--out PATH]` — the display state as one JSON doc.
    ...
    try:
        ...
        from altero.snapshot_json import take_snapshot, write_snapshot
...
def _swap_command(argv: list[str]) -> None:
    """Handle `altero swap NUM|EMAIL|ALIAS NUM|EMAIL|ALIAS`.
    ...
    parser = argparse.ArgumentParser(
        ...
        description=(
            ...
            "`altero list` and as numeric targets. Aliases, backups, and "
...
Examples:
  altero swap 1 2
  altero swap dev user@example.com
        """,
    )
...
def _move_command(argv: list[str]) -> None:
    """Handle `altero move NUM|EMAIL|ALIAS SLOT`.
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  altero move user@example.com 1   move an account onto shortcut 1
  altero move dev 1                by alias
  altero move 2 1                  by number (swaps if slot 1 is taken)
        """,
    )
...
def _alias_command(argv: list[str]) -> None:
    """Handle `altero alias [NUM|EMAIL] [NAME] [--unset]`.
...
Examples:
  altero alias 2 dev
  altero alias user@example.com dev
  altero alias 2 --unset
  altero alias                         # list all aliases
        """,
    )
    parser.add_argument(
        "account",
...
Examples:
  altero auto                       # foreground loop, switch at 90%% used
  altero auto --threshold 80        # switch earlier
  altero auto --model Fable         # also switch when the Fable weekly limit is hit
  altero auto --json                # one JSON event per line (for scripts)
  altero auto --once; echo $?       # single tick, outcome in exit code
  altero auto --dry-run             # log decisions, never actually switch
...
`altero config set autoswitch.enabled false` (or any surface's toggle).
...
Defaults live in settings.json in the backup root; flags override them.
    ...
    # The three service flags moved to `altero service`; kept here as
    ...
    # a hand-run `altero auto` is the user's own loop and must never quit on
    # its own.
    parser.add_argument(
        "--backend",
        action="store_true",
    ...
    from altero.autoswitch import AutoSwitchEngine, AutoSwitchEvent
    from altero.printer import accent, yellowed
    from altero.settings import load_settings, merged_with_cli
    ...
    try:
        ...
        if args.backend:
            from altero import launch_agent, widget_requests
        ...
        if code and args.backend:
            ...
            # almost always a hand-run `altero auto`. Exit 0: under KeepAlive
            ...
            print(
                "backend: another altero engine holds the engine lock; exiting "
...
# grace covers `altero menubar`, which starts the backend and then the menu
...
def _apply_widget_requests(engine, backup_dir: Path) -> None:
    ...
    try:
        # A fresh switcher, as `altero switch <N>` builds one: the engine's
...
def _config_command(argv: list[str]) -> None:
    """Handle `altero config [list|get KEY|set KEY VALUE|unset KEY|path]`.
    ...
    loudly here instead of silently degrading at `altero auto` time.
    """
    ...
    parser = argparse.ArgumentParser(
        prog="altero config",
        description=(
            "Read and edit altero settings (settings.json in the "
            "backup root)."
        ),
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=f"""
...
Examples:
  altero config                              # list effective settings
  altero config get autoswitch.threshold
  altero config set autoswitch.threshold 80
  altero config unset autoswitch.threshold   # back to the default
  altero config path                         # where settings.json lives
        """,
    )
    parser.add_argument(
        "--json",
    ...
    for p in (p_list, p_get):
        ...
        # pre-verb `altero config --json` in the shared namespace.
        p.add_argument(
            "--json",
            action="store_true",
...
def _menubar_start() -> int:
    """``altero menubar``: hand the menu bar to launchd and return the prompt.

    The foreground app is ``altero menubar --foreground``, which is what the
    ...
    """
    from altero import launch_agent
    from altero.menubar import framework_build_warning
    ...
    print(dimmed(
        ...
        "or run 'altero menubar --uninstall-service', to stop that."
    ))
    if unsupported:
...
def _menubar_uninstall() -> int:
    """``altero menubar --uninstall-service``: close the menu bar now and keep
    ...
    print(dimmed("Bring it back with: altero menubar"))
    return 0


def _service_command(argv: list[str]) -> None:
    """Handle `altero service [status|logs|start]`.
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  altero service status        # running? which build? who holds the engine?
  altero service logs -f       # follow the event stream
  altero service start         # start it without opening a surface
...
`altero config set autoswitch.enabled false`.
        """,
    )
    ...
    if sys.platform != "darwin":
        # status/logs read launchd state and ~/Library/Logs; neither exists
        # off macOS, where the engine runs in the foreground instead.
        error(
            "Error: The backend service is only available on macOS. On "
            "Windows and Linux, run `altero auto` (or the TUI) in a terminal."
        )
        sys.exit(1)
    try:
        if action == "logs":
...
def _service_install() -> int:
    ...
    print(
        dimmed(
            "It starts at login from now on. Re-run this after an altero "
    ...
    if not load_settings(paths.get_backup_root()).enabled:
        ...
        print(
            dimmed(
                ...
                "'altero config set autoswitch.enabled true'."
            )
        )
    return 0
...
def _service_start() -> int:
    """``altero service start``: ensure the backend, register no surface.

    The same ``open_surface`` call ``altero menubar`` makes with no kind, so
...
def _service_status() -> int:
    ...
    if not result["installed"] and not result["loaded"]:
        ...
        print(dimmed("It starts with the TUI ('altero') or the menu bar ('altero menubar')."))
    else:
    ...
    # `altero auto` or a menu bar holds it just as a LaunchAgent does.
...
def _service_logs(lines: int, follow: bool) -> int:
    ...
    if not out_log.exists():
        ...
        print(dimmed("The backend starts with the TUI ('altero') or the menu bar ('altero menubar')."))
        return 1
...
def _render_log_line(line: str) -> str:
    """Print one backend log line the way `altero auto` would have printed it.
    ...
    through untouched.
    """
    from altero.autoswitch import parse_event_line
...
def _auto_service(args) -> int:
    ...
    flag, replacement, action = (
        ("--install-service", "open 'altero' or 'altero menubar'", _service_install)
        if args.install_service
        else ("--uninstall-service", "close the TUI and the menu bar", _service_uninstall)
        if args.uninstall_service
        else ("--service-status", "use 'altero service status'", _service_status)
    )
    warning(
        f"'altero auto {flag}' is deprecated; {replacement} instead",
        file=sys.stderr,
    )
    return action()
...
def main() -> None:
    ...
    argv = sys.argv[1:]
    try:
        from altero.appearance import cli_should_probe, cli_theme
    ...
    # Bare `altero` in an interactive terminal opens the TUI dashboard (like
    # lazygit/k9s). TTY-gated on both ends so scripts and pipes keep getting
    # the usage error, and `altero tui` stays the explicit spelling.
    if not argv and sys.stdout.isatty() and sys.stdin.isatty():
        argv = ["--tui"]

    # Memorable subcommands (`altero switch <email>`, `altero list`, `altero help`, ...)
    ...
    parser = argparse.ArgumentParser(
        ...
        description="""Altero for Claude Code: multi-account switching
...
Commands:
  ...
  %(prog)s purge                      remove all altero data
...
The original flag spellings (%(prog)s --switch, %(prog)s --list, ...) keep working.
    ...
    # `altero menubar` hands the app to launchd and returns; this is the app.
    parser.add_argument(
        "--foreground",
        action="store_true",
        help=argparse.SUPPRESS,
    )
    # The spelling the published package already has for this (and `altero
    ...
    # no truthiness-tested branch of the dispatch chain below, so altero would
    # fall out of it and exit 0 having done nothing: `altero export "$DEST"` with
    ...
    if args.upgrade:
        from altero.update_check import run_self_upgrade
    ...
    # init-time failures are presented like every other ClaudeSwitchError: clean stderr line,
    # exit 1, no traceback.
    ...
    try:
        ...
        if args.add_account:
            ...
            from altero.settings import load_settings, parse_model_names
            ...
            from altero.menubar import run as menubar_run
    ...
    if not args.purge and not args.upgrade and not args.json:
        from altero.update_check import check_for_update
