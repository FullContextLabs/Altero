"""Directory → account mappings for `altero run` auto-resolution.

Maps a normalized absolute directory path to a stored account identity
(email + organizationUuid). `altero run` with no account argument resolves the
...
from altero.models import get_timestamp
