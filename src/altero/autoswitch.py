...
(so cron-driven ``altero auto --once`` ticks behave across processes), mutated
...
from altero.json_output import SCHEMA_VERSION, USAGE_TOKEN_EXPIRED, iso_timestamp
...
from altero.poll_policy import (
...
from altero.snapshot_json import history_for, snapshot_payload, write_snapshot
from altero.snapshot_source import SnapshotSource
from altero.switcher import ClaudeAccountSwitcher
from altero.usage_history import UsageHistory
from altero.usage_store import due_candidate, plan_oversleeps_interval

STATE_FILENAME = "autoswitch_state.json"
STATE_LOCK_FILENAME = ".autoswitch_state.lock"
STATE_SCHEMA_VERSION = 1

_logger = logging.getLogger("altero")
...
_SYSTEMIC_MESSAGES = {
    "store-unmirrored": "CLAUDE_SECURESTORAGE_CONFIG_DIR is set — unset it or "
                        "run altero from a normal shell",
    "invalid_client": "altero's OAuth client was rejected — systemic, not this "
                      "account",
    "stash-unreadable": "a stashed successor is unreadable — unlock the "
                        "keychain or fix the file, then retry; "
                        "`altero unclaimed` inspects it",
    "consume-busy": "another altero surface holds the slot — retries next pass",
}
...
def _recovery_is_useful(
    ...
    Why EITHER and not BOTH: requiring both would refuse the wait-for-reset case this
    ...
    multiple ticks, both rules were checked against both shapes:
        ...
        rule    oscillating pair        wait-for-reset pair
        cand    moves, moves back       moves, moves back   (the bug)
        both    never moves             never moves         (breaks wait-for-reset)
        either  moves, then holds       moves, then holds   (wanted)

    The wait-for-reset case oscillated on the original code too — its test ticks once,
...
class PollEvent(AutoSwitchEvent):
    ...
    # (e.g. "89%") hides which window binds — a real report came off that
    # ambiguity.
...
class QuarantineEvent(AutoSwitchEvent):
    ...
    def human(self) -> str:
        return (
            ...
            f"Log in with it and run 'altero --add-account --slot {self.number}' "
            "to recover."
        )
...
def parse_event_line(line: str) -> BackendEvent | None:
    """One JSONL line from ``altero auto --json``, or None if it isn't one.
...
def _window_pcts(
    ...
    correctly ignored it. Full per-model usage lives in ``altero list``.
    """
    return {
...
def record_manual_switch(
    ...
    Must be called with no other altero lock held: the engine takes this lock
...
class AutoSwitchEngine:
    ...
    def _ensure_engine_lock(self) -> bool:
        ...
        (``run_loop`` and a one-shot ``tick``, i.e. ``altero auto --once``
        ...
        self._emit(
            ErrorEvent(
                message=(
                    "another altero engine already owns this machine's accounts: "
    ...
    def _freshen_target(self, number: str, email: str) -> str:
        ...
        if self.switcher.live_session_pids_for(number, email):
            # A live `altero run` session owns this account's token in its own
    ...
    def _reload_settings(self) -> AutoSwitchSettings:
        ...
        ``altero config set`` must reach a service running since login without
    ...
    def _start_snapshot_publisher(self) -> None:
        ...
        self._snapshot_thread = threading.Thread(
            target=self._snapshot_loop, name="altero-snapshot", daemon=True
        )
    ...
    def _publish_snapshot(self) -> None:
        ...
        (``altero auto`` looping), write it here when none does (``--once``)."""
        ...
        try:
            ...
            write_snapshot(
                ...
                snapshot_payload(
                    ...
                    altero_command=resolve_program(),
                ),
            )
    ...
    def _tick_inner(self) -> TickOutcome:
        ...
        if current is None:
            ...
            if self.switcher.has_live_login():
                # Live login exists but altero doesn't manage it: never act —
                # a switch would overwrite it without a backup.
                self._emit(
                    NoSwitchEvent(
                        reason="unmanaged-active-account",
                        detail="run 'altero --add-account' to include it in rotation",
                    )
                )
            else:
                self._emit(
                    NoSwitchEvent(
                        ...
                        detail="log in and run 'altero --add-account' first",
                    )
                )
    ...
    def _perform(
        ...
        # switch path (altero FileLock + Claude Code locks) never takes the
        # state lock.
        with self._state_lock():
            state = self._read_state()
            if trigger in ("proactive", "consume-first") and self._in_cooldown(state):
