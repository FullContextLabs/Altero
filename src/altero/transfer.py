"""Export and import account data for altero.
...
(e.g. `altero --export - | gpg -c > out.gpg`).
"""
...
from altero.credentials import looks_like_api_key
...
from altero.models import Platform, get_timestamp, normalize_alias
...
def _slim_credentials(creds_obj: dict) -> dict:
    ...
    replaced from the live credential at activation anyway — or
...
def export_accounts(
    ...
    if not sequence_data or not sequence_data.get("accounts"):
        raise TransferError("no accounts to export — run altero --add-account first")
    ...
    if not accounts_payload:
        raise TransferError(
            ...
            "credentials/config. Re-add with: altero --add-account --slot <number>"
        )
...
def import_accounts(
    ...
    if source == "-":
        try:
            ...
            # ``altero --export - | gpg -c > backup.gpg`` is a documented
            ...
            raise TransferError(
                ...
                "first (e.g. gpg -d backup.gpg | altero --import -)"
            ) from e
    else:
        ...
        try:
            ...
            raise TransferError(
                ...
                "decrypt it first (e.g. gpg -d backup.gpg | altero --import -)"
            ) from e
        except OSError as e:
    ...
    if envelope.get("encrypted") is True:
        raise TransferError(
            ...
            "decrypt before piping (e.g. gpg -d backup.gpg | altero --import -)"
        )
    ...
    for entry in normalized:
        ...
        if existing_slot is not None:
            if force:
                ...
                # lifted. Silence here made a lifted-then-
                # honestly-re-condemned strike read as a clear that never happened.
                ...
                # Narrow auto-heal: a plain import replaces a
            ...
            # the next `altero run` re-bootstraps from the imported creds. A
            ...
            if live_pids:
                _eprint(
                    ...
                    "it is restarted via 'altero run'."
                )
        else:
        ...
        # never re-fetch to prove the imported token .
    ...
    # "I forced this" from "altero healed this". Appended only when it
    ...
    # credentials up over it — point at the explicit activation
    # path instead.
    ...
    if identity is not None and final is not None:
        ...
        if live_slot is not None and live_slot in written_slots:
            _eprint(
                ...
                f"imported credentials with: altero --switch-to {live_slot} --force"
            )
