...
from altero.exceptions import TransferError
from altero.models import Platform
from altero.oauth import credential_fingerprint
from altero.switcher import ClaudeAccountSwitcher
from altero.transfer import export_accounts, import_accounts
...
class TestRoundTrip:
    def test_export_import_round_trip(self, temp_home: Path):
        ...
        out_file = temp_home / "backup.altero"
...
class TestConflictPolicy:
    ...
    def test_import_over_live_login_hints_force_activation(
        ...
        --switch-to --force activation hint: a plain
        ...
        assert "altero --switch-to 1 --force" in err
...
class TestValidation:
    ...
    def test_missing_file_rejected(self, temp_home: Path):
        ...
        with pytest.raises(TransferError, match="not found"):
            import_accounts(s, str(temp_home / "nope.altero"))

    def test_still_encrypted_file_rejected(self, temp_home: Path):
        """`altero export - | gpg -c > backup.gpg` is documented, so importing
        ...
        s = _linux_switcher(temp_home)
        blob = temp_home / "backup.altero.gpg"
...
class TestPipeMode:
    ...
    def test_import_from_stdin_rejects_binary(self, temp_home: Path):
        """`cat backup.gpg | altero import -` is the pipe-mode form of the same
...
class TestSlimVsFullCredentials:
    """Default exports carry only the account's own login.
...
# ---------------------------------------------------------------------------
# tolerate broken slots in export
# ---------------------------------------------------------------------------


class TestExportSkipsBrokenSlots:
    """--export (all accounts) should warn-and-skip slots whose
    ...
    def test_explicit_account_with_missing_credentials_hard_fails(
        ...
        from altero.exceptions import CredentialReadError

        with pytest.raises(CredentialReadError, match="no backup credentials"):
            export_accounts(
                s, str(temp_home / "x.altero"), account="1"
            )
    ...
    def test_stdout_pipe_mode_keeps_stdout_pure_json(
        ...
        """altero --export - must produce valid JSON on stdout even when one
...
class TestImportSessionInvalidation:
    ...
    def test_force_overwrite_invalidates_session_credentials(
        ...
        # Credential material dropped → next `altero run` re-bootstraps from
...
class TestImportClearsDeadTokenQuarantine:
    ...
    credential can be re-tested. Assertions are on the
    ...
    def test_import_force_lifts_quarantine(self, temp_home: Path):
        ...
        lifts the verdict (the reported workflow)."""
        s = _linux_switcher(temp_home)
        _seed_account(s, 2, "bob@example.com")
    ...
    def test_force_import_of_the_byte_identical_struck_generation_lifts_the_quarantine(
        ...
        """Pins a reported experiment: the strike is bound to the
        stored generation's fingerprint and the forced import
    ...
    def test_reimport_after_removal_lifts_orphan_quarantine(
        ...
        """The case the overwrite-only fix would miss: `altero remove` never
        ...
        # Simulate `altero remove 2`: drop the sequence entry, leave the
    ...
    def test_plain_import_replaces_quarantined_slot(
        ...
        """Narrow auto-heal: a plain import (no --force)
    ...
    def test_the_heal_finds_the_row_of_an_org_scoped_account(
        ...
        fires. The slot the user was told to fix with `altero import` gets
    ...
    def test_import_heals_an_active_slot_struck_on_its_live_generation(
        ...
        reads it as healthy and refuses to replace it — and `altero import` is
    ...
    def test_plain_import_heal_warns_about_live_session(
        ...
        keeps its own credential copy until restarted via `altero run`."""
        import os as _os
...
class TestForceOverwriteNarratesTheStrikeClear:
    ...
    honestly re-condemn it. the silent lift → poll → re-strike
