"""Tests for one-time data migrations (altero.migrations).
...
keyring`` locally, patching ``altero.switcher.keyring`` would NOT affect
...
from altero.switcher import KEYRING_SERVICE, ClaudeAccountSwitcher
...
class TestWindowsFileBackend:
    def test_round_trip_uses_files_not_keyring(self, temp_home):
        ...
        with patch("altero.switcher.keyring", create=True) as mock_keyring:
...
class TestMacosKeyringToSecurity:
    ...
    service (``altero``). The autouse guard fakes the security backend
