...
only — the auto-switch engine itself lives in ``altero.autoswitch`` and is
tested there.
"""
...
from altero.switcher import USAGE_API_KEY
...
def test_notification_identity_creates_and_preserves_info_plist(tmp_path: Path):
    ...
    assert data["CFBundleIdentifier"] == "com.fullcontextlabs.altero.menubar"
    assert data["CFBundleName"] == "Altero"
...
def test_run_without_rumps_raises_clean_error(monkeypatch):
    ...
    around ``from altero.menubar import run`` can never fire — the import
    ...
    with pytest.raises(ClaudeSwitchError, match=r"altero\[menubar\]"):
...
class TestFrameworkBuildWarning:
    ...
    The interpreter *version* is not the variable — measured with the
...
# from the launchd label alone, so a hand-run `altero auto` holding the lock
...
def test_engine_owner_label_names_a_foreign_holder_rather_than_claiming_it():
    ...
    label = menubar.engine_owner_label(
        launch_agent.ENGINE_OTHER, "pid 4242 (/usr/local/bin/altero auto)"
    )
    assert label == "Engine: pid 4242 (/usr/local/bin/altero auto)"
