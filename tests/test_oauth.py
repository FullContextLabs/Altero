...
class TestFormatReset:
    ...
    def test_same_day_shows_time_only(self):
        ...
        with patch("altero.oauth.datetime") as mock_dt:
...
class TestFetchUsage:
    ...
    def test_success(self):
        ...
        with patch("altero.oauth.urllib.request.urlopen", return_value=mock_response), \
             patch("altero.oauth.datetime") as mock_dt:
            mock_dt.fromisoformat = datetime.fromisoformat
            mock_dt.now.return_value = fixed_now
    ...
    def test_network_error(self):
        with patch("altero.oauth.urllib.request.urlopen", side_effect=Exception("timeout")):
            result = oauth.fetch_usage("sk-test-token")
        assert result is None

    def test_http_error_logs_in_debug_mode(self, capsys):
        import logging
        logger = logging.getLogger("altero")
        ...
        try:
            ...
            with patch("altero.oauth.urllib.request.urlopen", side_effect=http_error):
                result = oauth.fetch_usage("sk-test-token")
...
class TestRefreshOAuthCredentials:
    ...
    def test_refresh_sends_correct_body(self):
        ...
        with patch("altero.oauth.urllib.request.urlopen", side_effect=mock_urlopen):
...
class TestTryRefreshOAuthCredentials:
    ...
    def test_success_rotates_and_has_no_error(self):
        ...
        with patch(
            "altero.oauth.urllib.request.urlopen", return_value=mock_response
        ):
    ...
    def test_invalid_grant_body_on_400_is_permanent(self):
        ...
        with patch("altero.oauth.urllib.request.urlopen", side_effect=err):
            outcome = oauth.try_refresh_oauth_credentials(self._make_credentials())
    ...
    def test_network_error_is_transient(self):
        with patch(
            "altero.oauth.urllib.request.urlopen",
...
class TestFetchUsageForAccount:
    ...
    def test_valid_token_fetches_usage_without_refresh(self):
        ...
        with patch("altero.oauth.urllib.request.urlopen", side_effect=mock_urlopen), \
             patch("altero.oauth.refresh_oauth_credentials") as refresh_mock:
            result = oauth.fetch_usage_for_account(
                "1", "test@example.com", credentials,
                is_active=False,
            )
    ...
    def test_active_account_skips_refresh_even_when_expired(self):
        ...
        own refresh via a lockfile on ~/.claude/ that altero doesn't honor, so
        altero must never touch the active account's tokens.
        """
        now_ms = int(datetime.now(timezone.utc).timestamp() * 1000)
        credentials = self._make_credentials(expires_at=now_ms - 1_000)
    ...
    def test_persist_failure_logs_warning_with_recovery_hint(self, caplog, capsys):
        ...
        a recovery hint (re-run `altero --add-account`), not debug, AND prints
        ...
        warning_records = [
            ...
            if r.levelno == logging.WARNING and r.name == "altero"
        ]
        assert len(warning_records) == 1
        msg = warning_records[0].getMessage()
        assert "failed to persist" in msg
        assert "altero --add-account" in msg
        assert "1" in msg
        ...
        assert "altero --add-account" in captured.err
        assert captured.out == ""
...
class TestTryFetchUsageOutcome:
    ...
    def test_success_outcome(self):
        ...
        with patch("altero.oauth.urllib.request.urlopen", return_value=resp):
    ...
    def test_429_outcome_carries_retry_after(self, caplog):
        ...
        with (
            patch("altero.oauth.urllib.request.urlopen", side_effect=err),
        ...
        # across altero surfaces can drain — the log says what is happening.
...
class TestInvalidGrantPropagation:
    ...
    def test_proactive_refresh_invalid_grant_short_circuits(self):
        ...
        with patch("altero.oauth.try_refresh_oauth_credentials",
             ...
             patch("altero.oauth.request_usage_data") as usage:
            outcome = oauth.try_fetch_usage_for_account(
...
class TestCredentialFingerprint:
    """Identity fingerprints for stored credentials (provenance guard)."""
...
class TestFetchOauthProfile:
    ...
    def test_401_is_unresolvable_with_log_file_warning(self, caplog):
        ...
        with patch(
            "altero.oauth.urllib.request.urlopen", side_effect=err,
        ), caplog.at_level(logging.WARNING, logger="altero"):
            assert oauth.fetch_oauth_profile("sk-live") is None
        assert any(
...
class TestInvalidGrantTaxonomy:
    ...
    def _refresh_with_body(self, monkeypatch, code, body):
        ...
        monkeypatch.setattr(
            "altero.oauth.urllib.request.urlopen", raise_http
        )
...
class TestConsumeBusyIsDeterministic:
    ...
    def test_a_busy_gate_does_not_spend_a_doomed_request(self):
        ...
        with patch("altero.oauth.request_usage_data") as usage:
