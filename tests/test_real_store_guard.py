...
``~/.local/share/<data dir>`` were overwritten at 03:26 with the exact
...
``altero.paths`` roots — regardless of which thread performs it.
"""
...
from altero import paths, session
...
def test_control_b_and_c_real_store_write_is_refused(monkeypatch):
    ...
    write under the REAL ``altero.paths.get_backup_root()`` and must
    ...
    """
    from altero.exceptions import ClaudeSwitchError  # noqa: F401  (sanity import only)

    marker_name = ".altero-test-real-store-guard-probe-DELETE-ME"
...
def test_frozen_specs_cover_the_data_root_and_the_transcript_tree(
    monkeypatch, tmp_path, platform
):
    """The data root is protected recursively on both layouts, and transcripts
    land two levels under a ``~/.claude`` that is non-recursive on purpose.
    """
    home = tmp_path / "home"
    ...
    # CONTROL: the patch above really selected this layout.
    assert paths.get_backup_root() == (
        home / ".altero" if platform is Platform.MACOS
        else home / ".local" / "share" / "altero"
    )

    specs = conftest._freeze_real_store_specs()
    recursive_roots = {root for root, recursive in specs if recursive}

    assert paths.get_backup_root() in recursive_roots
...
def test_frozen_specs_include_the_ambient_xdg_override_backup_root(
    ...
    `altero.paths` resolves to under the environment as it actually is."""
    home = tmp_path / "home"
    home.mkdir()
    ...
    assert xdg / "altero" in recursive_roots, (
        "the XDG-override backup root must be protected, not only the "
        "cleared-env default ~/.local/share/altero"
    )
    assert home / ".local" / "share" / "altero" in recursive_roots, (
...
def test_layout_a_runtime_real_store_is_refused_and_unrelated_tmp_still_writes(
    ...
    assert runtime_real_store == xdg / "altero"  # sanity: the hole's target
...
def test_arbitrary_claude_config_dir_is_not_dropped_by_the_hint_prefilter(
    ...
    guess (``(".claude", "altero")``) — correct for the two DEFAULT
    ...
    ``_REAL_STORE_SPECS``.
    """
    root_dir = Path(tempfile.mkdtemp(prefix="fcl-c2-noclaude-"))
    try:
        home = root_dir / "home"
        home.mkdir()
        (home / ".claude").mkdir()
        ...
        assert not any(
            hint in str(target) for hint in (".claude", "altero")
...
def test_i1_os_rename_source_out_of_protected_root_is_refused(
    ...
    same shape a ``shutil.move`` of the store uses to
...
def test_i2_os_symlink_into_protected_root_is_refused(
    ...
    path a reader (Claude Code, altero itself) would trust as real store
...
def test_mkdir_exist_ok_true_does_not_swallow_the_refusal(
    ...
    # exists on every developer machine before altero ever runs in-process).
...
def test_c0_a_scratch_home_still_protects_the_os_account_home_store(monkeypatch, tmp_path):
    ...
    # answers `~/.altero`, so both passes resolve to the same
    ...
    # there is `~/.altero`, so this case failed on the one
