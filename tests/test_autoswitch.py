...
from altero import oauth, poll_policy
...
from altero.usage_history import UsageHistory
from altero.usage_store import FetchRecord, UsageEntry
from altero.models import Platform
from altero.settings import AutoSwitchSettings, set_setting
...
class EngineHarness:
    ...
    def __init__(self, temp_home: Path, **settings_kwargs):
        ...
        # platform (both its XDG branch and its ~/.altero branch honour
        # it) —
...
class TestEngineHarnessIsolation:
    ...
    distinct subtrees collapsed to one store under the
    ~/.altero path, and h1's seeded account silently became
    h2's). `Path.home()` is honoured by get_backup_root() on EVERY platform
    (both its XDG branch and its ~/.altero branch), so scoping there instead makes the guard, and this
...
class TestDecisionTable:
    ...
    def test_issue_115_strictly_better_candidate_switches(self, harness):
        # Regression: active bound by 5h (99%), candidate bound by
    ...
    def test_unmanaged_live_login_is_never_touched(self, temp_home):
        ...
        h.seed(1, "a@example.com")
        h.seed(2, "b@example.com")
        # The user logged in with an account altero doesn't manage.
...
class TestAdaptiveScheduler:
    ...
    def _harness(self, temp_home, monkeypatch, accounts=3, **settings_kwargs):
        monkeypatch.setattr("altero.switcher._FETCH_STAGGER_S", 0)
    ...
    def test_stale_candidate_plan_never_gates_the_active(
        ...
        # Role change outside an altero switch (e.g. manual login): the active
    ...
    def test_a_non_429_ask_is_bounded_at_its_own_trust_ceiling(
        ...
        the base branch introduced by this PR). The bound is now
        ...
        test.
        """
        from altero.usage_store import TRUST_MAX_AGE_S, _failure_backoff_s
    ...
    def test_an_expired_trust_does_not_turn_one_block_into_a_request_storm(
        ...
        to clear (re-measured 2026-08-03). The previous code spends two.
        ...
        this.
        """
        from altero.usage_store import RETRY_AFTER_MARGIN_S, FetchRecord
        ...
        assert requests <= 2, (
            f"{requests} requests inside one {block_s:.0f}s block — the previous code "
    ...
    def test_poll_never_scheduled_past_a_window_reset(self, temp_home, monkeypatch):
        ...
        from altero.autoswitch import RESET_SLACK_S
...
class TestFreshening:
    def test_near_expiry_target_is_refreshed_and_persisted(self, temp_home):
        ...
        with patch(
            "altero.autoswitch.oauth.try_refresh_oauth_credentials",
    ...
    def test_live_session_target_is_skipped_even_with_fresh_token(self, temp_home):
        # Auto never activates an account that has a live `altero run` session:
...
class TestEnabledIsReadFromDiskEachTick:
    ...
    ``altero auto --threshold`` flag is), so these build an engine with
    ...
    def test_a_flag_this_process_was_given_survives_the_re_read(self, temp_home):
        # `altero auto --threshold 60` must not evaporate on the first tick,
...
class TestSnapshotPublishing:
    ...
    def test_a_failing_write_leaves_the_tick_alone(self, temp_home):
        ...
        with patch(
            "altero.autoswitch.write_snapshot", side_effect=OSError("no space")
        ):
...
class TestSnapshotHeartbeat:
    ...
    def test_republishes_while_the_tick_sleeps(self, temp_home):
        ...
        with patch("altero.autoswitch.SNAPSHOT_PUBLISH_INTERVAL_S", 0.01), patch(
            "altero.autoswitch.write_snapshot", side_effect=publish
...
class TestSnapshotAutoswitchAndHistory:
    ...
    def test_published_file_says_how_to_run_this_altero(self, temp_home):
        """The widget's host app runs ``alteroCommand + ["service", "start"]``."""
        from altero.launch_agent import resolve_program

        h = self._harness(temp_home)
        self._tick(h, {"1": _usage(10), "2": _usage(40), "3": _usage(5)})
        assert self._payload()["alteroCommand"] == resolve_program()
...
class TestTokenIdentity:
    ...
    def test_dead_slot_quarantined_even_with_safety_copy_present(self, harness):
        """No automatic promotion (fail-open rework of the provenance guard):
        a dead slot is quarantined outright; safety copies are forensic
        material, and recovery is the documented /login + altero add."""
...
class TestConsumeFirstStrategy:
    ...
    def test_respects_cooldown(self, temp_home):
        ...
        h.events.clear()
        # Now a sooner account appears, but we're within cooldown.
        outcome = h.tick_with_usage({
...
class TestConsumeFirstDepartureRecordsItsOwnTrigger:
    ...
    def test_the_null_snapshot_answers_differently_by_recorded_trigger(self):
        from altero.autoswitch import AutoSwitchEngine
...
class TestRecoveryHorizon:
    ...
    The wait-for-reset rule ("go where quota returns first") was measured on minutes-scale
    ...
    def test_a_minutes_away_reset_still_wins(self, harness):
        """The wait-for-reset design case is unchanged: an 8-minute wait is worth 9
        points of headroom."""
        outcome = harness.tick_with_usage({
    ...
    def test_a_weekly_bound_active_does_not_refuse_a_peer_back_in_minutes(
        ...
        peer — the wait-for-reset case this horizon is supposed to preserve. The
...
class TestHorizonAxisDoesNotFlap:
    ...
    wall, which is the failure the wait-for-reset rule exists to prevent.
...
class TestEscapeBeforeTheLimitLands:
    ...
    appealing and is wrong — it hijacks the recovery ranking and the
...
class TestReviewFindings202:
    """Three defects found reviewing the wait-for-reset rule, each reproduced before fixing.
    ...
    def test_recovery_reads_the_binding_windows_reset(self, harness):
        ...
        from altero.autoswitch import _binding_recovery_ts
...
class TestFreshenRoutesThroughGate:
    ...
    def test_lock_contention_is_not_reported_as_network_trouble(
        ...
        `altero switch` overlap it happens routinely. Reporting it as "could not
        ...
        """
        from altero import oauth as oauth_mod
...
class TestSingletonEngineLock:
    ...
    backend; this lock also catches a hand-run `altero auto`, a menu bar and a
    ...
    def test_a_second_engine_refuses_to_tick(self, harness):
        ...
        assert "another altero engine" in message
    ...
    def test_run_loop_declines_instead_of_killing_its_host(self, harness):
        ...
        from altero.locking import EngineLock
...
class TestBackendEventStream:
    ...
    def test_a_dry_run_switch_stays_a_dry_run_across_the_stream(self):
        from altero.autoswitch import SwitchEvent, parse_event_line
    ...
    def _error(self, message):
        from altero.autoswitch import ErrorEvent

        return ErrorEvent(message=message)

    def test_first_poll_backfills_the_tail_then_follows(self, tmp_path):
        from altero.autoswitch import BackendEventLog

        log = tmp_path / "com.fullcontextlabs.altero.auto.log"
...
class TestManualSwitchesStartTheCooldown:
    ...
    def _switch_by_hand(h: EngineHarness, number: str) -> None:
        """What `altero switch N` runs, on the harness's own clock."""
    ...
    def test_the_engines_own_switch_is_unchanged(self, harness):
        with patch("altero.autoswitch.record_manual_switch") as record:
            outcome = harness.tick_with_usage({
                "1": _usage(95), "2": _usage(10), "3": _usage(50),
            })
        assert outcome is TickOutcome.SWITCHED
    ...
    def test_the_wait_is_longer_than_the_lock_default(self, harness):
        ...
        with patch("altero.autoswitch.FileLock", spy):
    ...
    def test_a_cooldown_that_cannot_be_recorded_is_announced(self, harness, capsys):
        ...
        harness.clock.now = time.time()
        with patch(
            "altero.autoswitch.record_manual_switch",
    ...
    def test_the_cli_switch_path_gets_it(self, harness):
        ...
        harness.clock.now = time.time()
        with patch(
            "altero.cli.ClaudeAccountSwitcher", return_value=harness.switcher
        ), patch.object(sys, "argv", ["altero", "switch", "2", "--json"]), \
