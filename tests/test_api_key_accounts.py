...
display, the ``altero run`` session guard, and export/import of raw keys.
"""
...
from altero.json_output import USAGE_API_KEY, usage_fields
from altero.models import Platform
from altero.paths import get_credentials_path, get_global_config_path
...
class TestATornConfigSurvivesAnOrdinarySwitch:
    ...
    it is what a plain `altero switch` does, and the success line is what makes
    it invisible.
    """
    ...
    def test_a_plain_switch_does_not_flatten_a_torn_config(
        ...
        # The switch lands — the previous code replaces a malformed config here on
    ...
    def test_config_torn_between_the_switch_start_and_step_4_survives(
        ...
        assert out["switched"] is True, (
            ...
            "switch that the previous code would otherwise complete"
        )
        assert out["warnings"], (
    ...
    def test_a_failed_salvage_aborts_instead_of_flattening_the_config(
        ...
        with patch("altero.switcher.shutil.copy", side_effect=no_space):
...
class TestTheSalvageKeepsItsPromise:
    ...
    def test_the_salvage_does_not_widen_the_config_mode(self, temp_home: Path):
        ...
        the salvage kept 0644 and held `primaryApiKey` — altero created a
