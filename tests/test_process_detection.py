...
from altero.process_detection import (
    PID_REUSE_SLACK_S,
    ClaudeSession,
    IdeInstance,
    _lstart_seconds,
...
from altero.printer import abbreviate_path, entrypoint_label, format_age
...
class TestIsPidAlive:
    ...
    def test_alive_pid(self):
        with patch("altero.process_detection.sys.platform", "linux"), \
    ...
    def test_windows_dispatches_to_ctypes_impl(self):
        ...
        with patch("altero.process_detection.sys.platform", "win32"), \
             patch(
                 "altero.process_detection._is_pid_alive_windows",
                 return_value=True,
             ) as mock_win:
            assert is_pid_alive(12345) is True
...
class TestProcessStartedAt:
    def test_windows_is_unknowable(self):
        with patch("altero.process_detection.sys.platform", "win32"), \
             patch("altero.process_detection.subprocess.run") as run:
            assert process_started_at(1234) is None
        run.assert_not_called()

    def test_ps_failure_is_unknowable(self):
        with patch("altero.process_detection.sys.platform", "linux"), \
             patch("altero.process_detection.subprocess.run",
    ...
    def test_unknown_pid_is_unknowable(self):
        ...
        with patch("altero.process_detection.sys.platform", "linux"), \
             patch("altero.process_detection.subprocess.run", return_value=proc):
            assert process_started_at(1234) is None
...
class TestPidMatchesRecord:
    ...
    def test_unstamped_or_garbage_record_passes(self, proc_start):
        with patch("altero.process_detection.process_started_at") as started:
            assert pid_matches_record(1234, proc_start) is True
        started.assert_not_called()

    def test_unknowable_start_passes(self):
        with patch("altero.process_detection.process_started_at",
                   return_value=None), \
             patch("altero.process_detection.process_is_claude") as is_claude:
    ...
    def test_same_start_ticks_is_the_recorded_process(self):
        ...
        with patch("altero.process_detection.process_start_ticks",
                   return_value=TICKS), \
             patch("altero.process_detection.process_started_at") as started:
    ...
    def test_stranger_younger_than_record_is_a_recycled_pid(self):
        with patch("altero.process_detection.process_started_at",
             ...
             patch("altero.process_detection.process_is_claude",
                   return_value=False):
...
class TestListSessions:
    def test_reads_valid_sessions(self, tmp_path):
        ...
        with patch("altero.process_detection.is_pid_alive", return_value=True):
    ...
    def test_filters_dead_pids(self, tmp_path):
        ...
        with patch("altero.process_detection.is_pid_alive", side_effect=alive):
            result = list_sessions(tmp_path)
    ...
    def test_filters_recycled_pids(self, tmp_path):
        ...
        with patch("altero.process_detection.is_pid_alive", return_value=True), \
             patch("altero.process_detection.process_started_at",
                   side_effect=started), \
             patch("altero.process_detection.process_is_claude",
                   return_value=False):
            result = list_sessions(tmp_path)
    ...
    def test_filters_recycled_pids_by_start_ticks(self, tmp_path):
        ...
        with patch("altero.process_detection.is_pid_alive", return_value=True), \
             patch("altero.process_detection.process_start_ticks",
                   side_effect=ticks):
            result = list_sessions(tmp_path)
...
class TestListIdeInstances:
    ...
    def test_filters_dead_pids(self, tmp_path):
        ...
        with patch("altero.process_detection.is_pid_alive", side_effect=lambda p: p == 2001):
            result = list_ide_instances(tmp_path)
