"""Tests for altero.logging_config."""
...
def test_setup_does_not_create_dir(tmp_path: Path):
    ...
    The log dir lives under the Altero data root; a logger alone must not
    lay down directories there.
    """
...
def test_dir_is_created_on_first_log(tmp_path: Path):
    ...
    try:
        ...
        assert (log_dir / "altero.log").exists()
    finally:
        for handler in logger.handlers[:]:
            handler.close()
            logger.removeHandler(handler)
