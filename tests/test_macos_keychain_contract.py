...
1. **Mocked tests** (run on every PR, every platform): assert that the macOS
   ...
   `macos_keychain` security wrapper, under the new `altero` service. This
...
class TestBackupCredentialsSecurity:
    ...
    per-account backup service is the new ``altero`` (not the old keyring
    ``claude-code``).
    """
    ...
    def test_read_account_credentials_uses_security_service(
        ...
        with patch("altero.credentials.macos_keychain") as mock_kc:
            ...
            mock_kc.get_password.assert_called_once_with(
                "altero", "account-1-user@example.com"
            )
    ...
    def test_write_account_credentials_uses_security_service(
        ...
        with patch("altero.credentials.macos_keychain") as mock_kc:
            ...
            mock_kc.set_password.assert_called_once_with(
                "altero", "account-2-alice@example.com", "secret-token"
            )
    ...
    def test_write_retains_prev_generation_in_keychain_not_a_file(
        ...
        with patch("altero.credentials.macos_keychain") as mock_kc:
            ...
            mock_kc.set_password.assert_has_calls([
                call("altero", "account-2-alice@example.com.prev",
                     "old-generation"),
                call("altero", "account-2-alice@example.com",
                     "secret-token"),
            ])
    ...
    def test_delete_account_credentials_uses_security_service(
        ...
        with patch("altero.credentials.macos_keychain") as mock_kc:
            ...
            mock_kc.delete_password.assert_has_calls([
                call("altero", "account-3-bob@example.com"),
                call("altero", "account-3-bob@example.com.prev"),
                call("altero", "account-None-bob@example.com"),
                call("altero", "account-None-bob@example.com.prev"),
            ])
...
def test_wrapper_roundtrip_real_keychain(tmp_keychain: str):
    ...
    """
    macos_keychain.set_password("altero-test", "acct-1", "round-trip-token")
    assert macos_keychain.get_password("altero-test", "acct-1") == "round-trip-token"
    macos_keychain.delete_password("altero-test", "acct-1")
    assert macos_keychain.get_password("altero-test", "acct-1") is None
...
class TestOurOwnFileModeIsNotAKeychainFailure:
    ...
    def test_a_write_fallback_does_not_certify_an_unread_backup(
        ...
        """
        from altero import macos_keychain as _kc
    ...
    def test_a_write_fallback_does_not_certify_the_ACTIVE_read_either(
        ...
        sends the user to `altero --add-account`, the one remedy that cannot
    ...
    def test_the_default_capture_path_refuses_a_degraded_read(
        ...
        from altero.paths import get_credentials_path
    ...
    def test_a_recovered_keychain_does_not_latch_through_a_later_pin(
        ...
        `altero add`, `_resync_rotated_backup` never runs.
    ...
    def test_a_recovered_keychain_does_not_latch_through_an_UNVERIFIED_pin(
        ...
        A latch means `_refuse_degraded_capture` blocks `altero add`,
    ...
    def test_a_verified_clear_ends_the_degraded_verdict(
        ...
        `altero add` forever, and `_fetch_active_usage` returns the
...
class TestAnUnverifiedPinStaysPinned:
    ...
    def test_the_unverified_residual_is_not_served_after_the_cooldown(
        ...
        from altero import macos_keychain as _kc
        from altero.credentials import (
