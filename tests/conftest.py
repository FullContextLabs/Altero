...
from altero import macos_keychain as _macos_keychain
from altero import paths as _paths
...
class RealStoreWriteBlocked(Exception):
    ...
    Measured incident: the real ``~/.local/share/<data dir>/sequence.json``
...
def _freeze_real_store_specs() -> tuple[tuple[Path, bool], ...]:
    ...
    compares candidate write targets against. Re-resolving ``altero.
    ...
    * The GENUINELY-DEFAULT roots: mirrors ``_isolate_real_home``'s notion of
      ...
      altero``) protected — the same three vars that fixture neutralizes
      for every test, regardless of ``temp_home``.
    * The AMBIENT-OVERRIDE roots: resolved WITHOUT clearing those vars, i.e.
      exactly what ``altero.paths`` resolves to under the environment as
    ...
    ``recursive=True`` roots (the altero backup root, current-XDG and legacy)
    are exclusively altero's own data — everything beneath them is protected,
    any depth. This applies to an override-derived backup root too: it's
    still altero's own data regardless of which env var pointed at it.

    ``recursive=False`` roots are directories altero shares with unrelated
    ...
    files ``altero.paths``/``claude_locks`` ever place directly inside
...
def _derive_real_store_hints(
    ...
    substring scan, not a function call into altero.paths.

    C2: this used to be a hardcoded guess, ``(".claude", "altero")`` —
    correct for the two DEFAULT roots (the XDG backup root always ends in
    ``/altero``, the config home in ``/.claude``), but a developer with
...
_GUARD_PROBE_MARKER = ".altero-test-real-store-guard-probe-DELETE-ME"
...
def _real_store_audit_hook(event: str, args: tuple) -> None:
    ...
    if event == "open":
        ...
        # what a directory move uses) relocates the protected
...
def _isolate_real_home(request, tmp_path_factory, monkeypatch):
    ...
    that switcher resolves to the real ``~/.altero`` — writing logs,
...
def block_real_keychain(request, monkeypatch):
    ...
    Replaces the ``security``-CLI wrapper (``altero.macos_keychain``) with an
...
def block_real_oauth_profile_fetch(request, monkeypatch):
    ...
    if request.node.get_closest_marker("no_oauth_profile_fake"):
        yield
        return
    monkeypatch.setattr("altero.oauth.fetch_oauth_profile", lambda token: None)
    yield
...
def _deterministic_poll_jitter(monkeypatch):
    ...
    monkeypatch.setattr("altero.poll_policy.JITTER_FRAC", 0.0)
...
def _deterministic_colour(monkeypatch):
    ...
    from altero import appearance as _appearance
    from altero import printer as _printer
    ...
    monkeypatch.setattr("altero.printer._colors_enabled", None)
    ...
    # FORCE_COLOR.
    monkeypatch.setattr("altero.printer._theme", "dark")
    ...
    monkeypatch.setattr("altero.appearance._cache", _appearance._UNSET)
