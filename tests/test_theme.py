...
from altero.tui import theme
from altero.tui.theme import ALTERO_DARK, ALTERO_LIGHT, Palette
...
def test_from_theme_reads_theme_object_including_track():
    p = Palette.from_theme(ALTERO_LIGHT)
...
def test_both_themes_expose_track_variable():
    assert ALTERO_DARK.variables["track"] == theme.TRACK
    assert ALTERO_LIGHT.variables["track"] == theme.TRACK_LIGHT
    assert ALTERO_LIGHT.dark is False
