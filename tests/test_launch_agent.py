"""Tests for the launchd LaunchAgent that keeps ``altero menubar`` alive.
...
from altero import __version__, launch_agent
from altero.exceptions import ClaudeSwitchError

PROGRAM = ["/Users/x/.local/bin/altero"]
UID = 501
...
def test_build_plist_is_parseable_and_runs_the_foreground_menubar(tmp_path):
    # --foreground: plain `altero menubar` installs this agent and returns, so
...
def test_build_plist_path_env_includes_the_user_bin_dir(tmp_path):
    # A uv tool install puts altero's siblings in ~/.local/bin; launchd's own
...
def test_build_plist_runs_whatever_subcommand_it_is_given(tmp_path):
    ...
    assert parsed["Label"] == "com.fullcontextlabs.altero.auto"
    assert parsed["ProgramArguments"] == [*PROGRAM, "auto"]
    assert parsed["StandardErrorPath"] == str(
        tmp_path / "Library/Logs" / "com.fullcontextlabs.altero.auto.err"
    )
...
def test_resolve_program_keeps_the_symlink_and_does_not_follow_it(tmp_path):
    """`uv tool install` links ~/.local/bin/altero into the tool's virtualenv.
    ...
    real = venv_bin / "altero"
    real.write_text("#!/bin/sh\n")
    link_dir = tmp_path / "local" / "bin"
    link_dir.mkdir(parents=True)
    link = link_dir / "altero"
...
def test_resolve_program_makes_a_relative_argv0_absolute(tmp_path, monkeypatch):
    ...
    with patch.object(launch_agent.sys, "argv", ["./altero"]):
...
def test_resolve_program_falls_back_to_the_interpreter_without_a_script(tmp_path):
    with patch.object(launch_agent.sys, "argv", [str(tmp_path / "gone")]):
        with patch.object(launch_agent.shutil, "which", return_value=None):
            assert launch_agent.resolve_program() == [sys.executable, "-m", "altero"]


def test_resolve_program_ignores_an_argv0_that_is_not_altero(tmp_path):
    # Running through pytest, argv[0] is the test runner — not a thing launchd
    # should be pointed at.
    other = tmp_path / "pytest"
    other.write_text("#!/bin/sh\n")
    found = tmp_path / "altero"
...
def test_status_reads_the_jobs_own_state_not_a_nested_blocks(tmp_path):
    ...
    """
    printed = (
        "gui/501/com.fullcontextlabs.altero.menubar = {\n"
        "\tactive count = 1\n"
        "\tstate = running\n"
        "\tpid = 25026\n"
        "\tpid-local endpoints = {\n"
...
def test_engine_owner_calls_a_foreign_holder_what_it_is(tmp_path):
    ...
    _seed_owner(tmp_path, pid=4242, program="/usr/local/bin/altero auto")
    with patch.object(launch_agent, "backend_pid", lambda: None):
        state, detail = _owner_while_held(tmp_path)
    assert state == launch_agent.ENGINE_OTHER
    assert detail == "pid 4242 (/usr/local/bin/altero auto)"
...
def _owner_while_held(backup_dir: Path):
    ...
    """
    from altero.locking import FileLock, engine_lock_path
...
def _install_plist(
    ...
    if version is not None:
        body["EnvironmentVariables"] = {"ALTERO_VERSION": version}
...
class TestNeedsInstall:
    ...
    def test_another_build_in_the_plist_is_replaced(self, tmp_path):
        _install_plist(
            tmp_path, launch_agent.AUTO_LABEL, ["/elsewhere/altero", *self.ARGS]
        )
        with patch.object(launch_agent, "resolve_program", return_value=PROGRAM):
            with patch.object(launch_agent.subprocess, "run", side_effect=_running()):
    ...
    def test_an_upgrade_is_picked_up(self, tmp_path):
        # `altero upgrade` keeps the console script's path, so the argv still
...
class TestOpenAtLogin:
    ...
    def test_on_writes_the_menubar_plist(self, tmp_path):
        ...
        assert parsed["EnvironmentVariables"]["ALTERO_VERSION"] == __version__
        assert launch_agent.opens_at_login(tmp_path)
...
class TestTrimTheBackendLogs:
    """``~/Library/Logs/com.fullcontextlabs.altero.auto.{log,err}`` grow forever otherwise.
    ...
    def _log(self, tmp_path: Path, size: int) -> Path:
        path = tmp_path / "com.fullcontextlabs.altero.auto.log"
...
class TestPlacedWidgetsKeepTheBackend:
    ...
    def _app(self, home: Path, app: Path | None = None) -> Path:
        exe = (app or home / launch_agent.WIDGET_HOST_APP) / "Contents" / "MacOS" / "Altero"
        exe.parent.mkdir(parents=True)
        exe.write_text("#!/bin/sh\n")
        return exe
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            subprocess.TimeoutExpired(["Altero"], 10),
