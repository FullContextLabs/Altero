"""Tests for session mode (altero.session + the switcher guards)."""
...
from altero import session as session_mod
from altero.credentials import CLAUDE_CODE_MANAGED_KEYCHAIN_SERVICE
...
from altero.paths import get_global_config_path
...
class TestBootstrap:
    ...
    def test_stale_marker_forces_rebootstrap_after_session_exits(
        ...
        that motivates it), and a profile marked by an older altero on this
...
class TestIsSessionValid:
    ...
    def test_probe_timeout_accepts_keychain_only_credentials(
        ...
        profile's freshest token family.
        """
        tmp_path.mkdir(exist_ok=True)
        self._seed_profile(tmp_path)
        (tmp_path / ".credentials.json").unlink()
    ...
    def test_probe_timeout_still_rejects_drifted_identity(
        ...
        as; on timeout that local record still gates validity.
        """
        tmp_path.mkdir(exist_ok=True)
    ...
    def test_probe_timeout_lenient_on_unreadable_identity(
        ...
        cleanup path that was removed: a backup whose oauthAccount lacks an
...
class TestRun:
    ...
    def test_auth_override_vars_scrubbed_from_session_env(
        ...
        # `altero run 2` means account 2, not whatever the API key resolves to.
...
class TestGuards:
    ...
    def test_backup_credential_write_invalidates_stale_profile(
        ...
        recovery path leaves `altero run` on stale credentials that still pass
    ...
    def test_delete_session_profile_survives_a_denied_dir_with_legacy_marker(
        ...
        with caplog.at_level(logging.DEBUG, logger="altero"):
            try:
                seeded_switcher._delete_session_profile(ACCOUNT_NUM, ACCOUNT_EMAIL)
            finally:
                if deny:
    ...
    def test_list_skips_refresh_for_live_session_accounts(
        ...
        """altero --list must not proactively refresh an account that is live in
        ...
        monkeypatch.setattr(
            "altero.oauth.try_fetch_usage_for_account", fake_fetch
        )
    ...
    def test_unreadable_keychain_fails_closed(
        ...
        def locked(service: str, account: str) -> str | None:
            if not service.startswith("Claude Code-credentials-"):
                return fake_store_read(service, account)  # altero's own backup store
...
class TestBootstrapRefreshRoutesThroughGate:
    ...
    def test_bootstrap_uses_gate(self, temp_home, monkeypatch):
        ...
        monkeypatch.setattr(
            "altero.oauth.try_refresh_oauth_credentials", direct_post
        )
        monkeypatch.setattr(
            "altero.oauth.refresh_oauth_credentials", direct_post
        )
...
class TestAConsumedGrantIsNotSpentOnAProfileThatWonBootstrap:
    ...
    one). The under-lock re-check then returns early when another `altero run`
