"""Tests for altero.fsutil filesystem primitives."""
...
from altero.fsutil import read_text_with_retry, replace_with_retry
...
class TestReplaceWithRetry:
    ...
    def test_retries_transient_windows_errors(self, tmp_path, monkeypatch, winerror):
        monkeypatch.setattr("altero.fsutil.sys.platform", "win32")
        calls = []
        ...
        monkeypatch.setattr("altero.fsutil.os.replace", flaky)
    ...
    def test_gives_up_and_raises_after_attempts(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("altero.fsutil.os.replace", always_fail)
    ...
    def test_does_not_retry_real_errors(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("altero.fsutil.os.replace", missing)
        with pytest.raises(OSError):
            replace_with_retry(tmp_path / "a", tmp_path / "b")
        assert len(calls) == 1

    def test_posix_never_retries(self, tmp_path, monkeypatch):
        monkeypatch.setattr("altero.fsutil.sys.platform", "linux")
        calls = []
        ...
        monkeypatch.setattr("altero.fsutil.os.replace", fail)
        with pytest.raises(OSError):
            replace_with_retry(tmp_path / "a", tmp_path / "b")
        assert len(calls) == 1
