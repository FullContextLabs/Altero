"""Tests for the `altero config` subcommand (get/set/unset/list/path)."""
...
class TestConfigMisc:
    ...
    def test_main_help_mentions_config(self, temp_home, capsys):
        with patch.object(sys, "argv", ["altero", "--help"]):
    ...
    def test_auto_picks_up_configured_threshold(self, temp_home, capsys):
        """End-to-end: a value set via config drives `altero auto`."""
        ...
        with patch("altero.autoswitch.AutoSwitchEngine", FakeEngine), \
