"""Tests for `altero swap` (ClaudeAccountSwitcher.swap_accounts)."""
...
from altero.credentials import CredentialStore
...
class TestSwapAccounts:
    ...
    def test_swap_keeps_sequence_sorted(
        ...
        numbers — the accounts genuinely trade places in `altero list`."""
        switcher = ClaudeAccountSwitcher()
        self._write(switcher, sample_sequence_data)
    ...
    def test_write_json_publishes_only_after_chmod(
        ...
        # Scoped context: see H-1 comment above.
        with pytest.MonkeyPatch.context() as mp:
            mp.setattr("altero.switcher.os.chmod", failing_chmod)
            with pytest.raises(OSError):
    ...
    def test_swap_holds_account_lock(
        ...
        monkeypatch.setattr("altero.switcher.FileLock", SpyLock)
        switcher.swap_accounts("1", "2")
...
class TestSwapSessionProfileKeychain:
    ...
    the item behind at the old name. Nothing in altero ever names that service
