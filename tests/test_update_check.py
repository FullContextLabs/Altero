...
from altero.update_check import (
    CACHE_TTL,
...
def _published(monkeypatch):
    """The checks below exercise the live path; see test_unpublished_is_silent."""
    monkeypatch.setattr("altero.update_check.PUBLISHED_ON_PYPI", True)


def test_unpublished_is_silent(monkeypatch):
    monkeypatch.setattr("altero.update_check.PUBLISHED_ON_PYPI", False)
    with patch("altero.update_check.urllib.request.urlopen") as urlopen:
        assert check_for_update("0.0.1") is None
    urlopen.assert_not_called()


class TestCheckForUpdate:
    @patch("altero.update_check.urllib.request.urlopen")
    def test_newer_version_available(self, mock_urlopen, tmp_path, monkeypatch):
        monkeypatch.setattr("altero.update_check.CACHE_PATH", tmp_path / "cache.json")
    ...
    @patch("altero.update_check.urllib.request.urlopen", side_effect=OSError("network error"))
    def test_network_error_returns_none_and_caches(self, mock_urlopen, tmp_path, monkeypatch):
        cache_path = tmp_path / "cache.json"
        monkeypatch.setattr("altero.update_check.CACHE_PATH", cache_path)
    ...
    def test_fresh_cache_no_network(self, tmp_path, monkeypatch):
        ...
        with patch("altero.update_check.urllib.request.urlopen") as mock_urlopen:
            result = check_for_update("0.3.2")
            mock_urlopen.assert_not_called()
...
class TestCheckForUpdatePrereleases:
    """altero ships every cycle as a pre-release first, so the version
...
class TestDetectInstallMethod:
    def _set_prefix(self, monkeypatch, prefix: str) -> None:
        monkeypatch.setattr("altero.update_check.sys.prefix", prefix)
    ...
    def test_uv_tool_default_path(self, monkeypatch):
        self._set_prefix(monkeypatch, "/home/me/.local/share/uv/tools/altero")
        assert _detect_install_method() == "uv"

    def test_pipx_default_path(self, monkeypatch):
        self._set_prefix(monkeypatch, "/home/me/.local/pipx/venvs/altero")
    ...
    def test_source_checkout_returns_none(self, monkeypatch):
        self._set_prefix(monkeypatch, "/home/me/code/altero/.venv")
        assert _detect_install_method() is None
    ...
    def test_uv_tool_dir_env_with_prefix_under_it(self, monkeypatch, tmp_path):
        ...
        prefix = custom_root / "altero"
        monkeypatch.setattr("altero.update_check.sys.prefix", str(prefix))
    ...
    def test_uv_tool_dir_env_set_but_prefix_elsewhere(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            "altero.update_check.sys.prefix", str(tmp_path / "some-project" / ".venv")
        )
        monkeypatch.setenv("UV_TOOL_DIR", str(custom_root))
        monkeypatch.delenv("PIPX_HOME", raising=False)
        assert _detect_install_method() is None
    ...
    def test_pipx_home_env_with_prefix_under_it(self, monkeypatch, tmp_path):
        ...
        prefix = custom_root / "venvs" / "altero"
        monkeypatch.setattr("altero.update_check.sys.prefix", str(prefix))
...
class TestCheckForUpdateMessage:
    @patch("altero.update_check.sys.platform", "linux")
    @patch("altero.update_check.urllib.request.urlopen")
    def test_detected_method_non_windows_suggests_altero_upgrade(
        self, mock_urlopen, tmp_path, monkeypatch
    ):
        # uv/pipx on macOS/Linux: altero upgrade actually upgrades, so advertise it.
        monkeypatch.setattr("altero.update_check.CACHE_PATH", tmp_path / "cache.json")
        monkeypatch.setattr("altero.update_check._detect_install_method", lambda: "uv")
        mock_urlopen.return_value = _make_pypi_response("0.4.0")

        result = check_for_update("0.3.2")

        assert result is not None
        assert "altero upgrade" in result
        assert "uv tool upgrade" not in result

    @patch("altero.update_check.sys.platform", "win32")
    @patch("altero.update_check.urllib.request.urlopen")
    def test_detected_method_windows_suggests_direct_command(
        ...
        monkeypatch.setattr("altero.update_check.CACHE_PATH", tmp_path / "cache.json")
        monkeypatch.setattr("altero.update_check._detect_install_method", lambda: "pipx")
        mock_urlopen.return_value = _make_pypi_response("0.4.0")

        result = check_for_update("0.3.2")

        assert result is not None
        assert "pipx upgrade altero" in result
        assert "altero upgrade" not in result

    @patch("altero.update_check.urllib.request.urlopen")
    def test_unknown_method_suggests_altero_instructions(
        self, mock_urlopen, tmp_path, monkeypatch
    ):
        # Unknown install method: altero upgrade can only show instructions.
        monkeypatch.setattr("altero.update_check.CACHE_PATH", tmp_path / "cache.json")
        monkeypatch.setattr("altero.update_check._detect_install_method", lambda: None)
        mock_urlopen.return_value = _make_pypi_response("0.4.0")

        result = check_for_update("0.3.2")

        assert result is not None
        assert "altero upgrade` for upgrade instructions" in result
        assert "uv tool upgrade" not in result
...
class TestRunSelfUpgrade:
    @patch("altero.update_check.subprocess.run")
    @patch("altero.update_check._detect_install_method", return_value="uv")
    def test_uv_invokes_uv_tool_upgrade(self, mock_detect, mock_run):
        mock_run.return_value = MagicMock(returncode=0)

        assert run_self_upgrade() == 0
        mock_run.assert_called_once_with(
            ["uv", "tool", "upgrade", "altero"], check=False
        )

    @patch("altero.update_check.subprocess.run")
    @patch("altero.update_check._detect_install_method", return_value="pipx")
    def test_pipx_invokes_pipx_upgrade(self, mock_detect, mock_run):
        mock_run.return_value = MagicMock(returncode=0)

        assert run_self_upgrade() == 0
        mock_run.assert_called_once_with(
            ["pipx", "upgrade", "altero"], check=False
        )
    ...
    @patch("altero.update_check.subprocess.run")
    @patch("altero.update_check._detect_install_method", return_value=None)
    def test_unknown_method_returns_1_and_prints_instructions(
        ...
        assert "uv tool upgrade altero" in err
        assert "pipx upgrade altero" in err
        assert "pip install --upgrade altero" in err

    @patch(
        "altero.update_check.subprocess.run", side_effect=FileNotFoundError
    )
    @patch("altero.update_check._detect_install_method", return_value="uv")
...
class TestRunSelfUpgradeWindows:
    ...
    def test_uv_prints_command_and_does_not_run(self, mock_detect, mock_run, capsys):
        ...
        assert "uv tool upgrade altero" in out
    ...
    def test_pipx_prints_command_and_does_not_run(self, mock_detect, mock_run, capsys):
        ...
        mock_run.assert_not_called()
        out = capsys.readouterr().out
        assert "pipx upgrade altero" in out
