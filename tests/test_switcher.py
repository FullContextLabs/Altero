...
from altero import macos_keychain
from altero import oauth
from altero.json_output import USAGE_FOREIGN_CREDENTIAL, USAGE_TOKEN_EXPIRED
from altero.exceptions import (
...
from altero.usage_store import FetchRecord, UsageEntry, UsageStore
from altero.macos_keychain import KeychainError
from altero.models import Platform, normalize_alias
from altero.paths import get_backup_root, get_credentials_path
from altero.session import mark_session_stale
from altero.credentials import ActiveCredentials
from altero.switcher import (
...
class TestAliasCommand:
    ...
    def test_rename_via_existing_alias_identifier(
        ...
        """altero alias <old> <new> — identifier can itself be an alias."""
        switcher = ClaudeAccountSwitcher()
        self._write(switcher, sample_sequence_data)
...
class TestStatusCache:
    ...
    def test_status_uses_cached_usage(
        ...
        with patch.object(switcher, "_read_active_credentials",
             ...
             patch("altero.oauth.try_fetch_usage_for_account") as mock_fetch:
            switcher.status()
    ...
    def test_status_fetches_with_is_active_true_when_cc_running(
        ...
        with patch.object(switcher, "_read_active_credentials",
                          return_value=ActiveCredentials(active_creds, False)), \
             patch("altero.oauth.try_fetch_usage_for_account",
...
class TestFetchAccountUsageSessionProfile:
    ...
    def test_fresh_session_credentials_fetch_read_only(self, temp_home: Path):
        ...
        with patch.object(switcher, "_live_session_pids", return_value=[123]), \
             patch("altero.session.read_session_credentials",
    ...
    def test_exited_session_rejected_backup_still_refreshes(self, temp_home: Path):
        """With nobody live the backup is altero's to refresh: a 401 stays an
...
class TestAdoptSessionCredential:
    ...
    def test_stale_marked_profile_is_not_adopted(self, temp_home: Path):
        """The backup moved under this profile while it was live; altero has
...
class TestListAccountsUsage:
    ...
    def test_list_shows_usage(
        ...
        with patch.object(switcher, "_read_credentials", return_value=active_creds), \
             ...
             patch("altero.oauth.urllib.request.urlopen", return_value=mock_response):
    ...
    def test_list_shows_alias_before_email(
        ...
        with patch.object(switcher, "_read_credentials", return_value=active_creds), \
             patch.object(switcher, "_read_account_credentials", return_value=backup_creds), \
             patch("altero.oauth.try_fetch_usage_for_account", return_value=oauth.UsageOutcome(None)):
            switcher.list_accounts()
    ...
    def test_list_never_writes_live_while_claude_code_running(
        ...
        refresh (which coordinates via a ~/.claude/ lockfile altero doesn't honor) and
        ...
        with patch.object(switcher, "_read_credentials", return_value=active_creds), \
             ...
             patch("altero.oauth.try_fetch_usage_for_account", side_effect=mock_fetch):
            switcher.list_accounts()
    ...
    def test_list_shows_token_status_when_requested(
        ...
        with patch.object(switcher, "_read_credentials", return_value=active_creds), \
             patch.object(switcher, "_read_account_credentials", return_value=backup_creds), \
             patch("altero.oauth.try_fetch_usage_for_account", return_value=oauth.UsageOutcome(None)), \
             patch("altero.session.read_session_credentials", return_value=None), \
             patch("altero.oauth.build_token_status", return_value="oauth: fresh, refresh token yes"):
    ...
    def test_token_status_lines_for_active_account_use_active_profile(self, temp_home: Path):
        ...
        with patch("altero.oauth.build_token_status", return_value="oauth: fresh, refresh token yes") as build:
    ...
    def test_token_status_lines_preserve_api_key_silence(self, temp_home: Path):
        ...
        with patch("altero.session.read_session_credentials") as read_session, \
             patch("altero.oauth.build_token_status") as build:
            lines = switcher._token_status_lines(
    ...
    def test_token_status_lines_prefer_matching_session_profile_then_backup(self, temp_home: Path):
        ...
        with patch("altero.session.read_session_credentials", return_value="session-creds") as read_session, \
             patch("altero.session.session_identity_drifted", return_value=False) as drifted, \
             patch("altero.oauth.build_token_status", side_effect=build_status) as build:
            lines = switcher._token_status_lines(
    ...
    def test_token_status_lines_ignore_drifted_session_profile(self, temp_home: Path):
        ...
        with patch("altero.session.read_session_credentials", return_value="session-creds") as read_session, \
             patch("altero.session.session_identity_drifted", return_value=True) as drifted, \
             patch(
                 "altero.oauth.build_token_status",
    ...
    def test_token_status_lines_without_session_show_only_backup(self, temp_home: Path):
        ...
        with patch("altero.session.read_session_credentials", return_value=None) as read_session, \
             patch("altero.session.session_identity_drifted") as drifted, \
             patch(
                 "altero.oauth.build_token_status",
    ...
    def test_token_status_lines_are_read_only(self, temp_home: Path):
        ...
        with patch("altero.session.read_session_credentials", return_value="session-creds"), \
             patch("altero.session.session_identity_drifted", return_value=False), \
             patch(
                 "altero.oauth.build_token_status",
                 side_effect=["oauth: fresh, refresh token yes", "oauth: expired, refresh token yes"],
             ), \
             patch("altero.oauth.try_refresh_oauth_credentials") as refresh, \
             patch("altero.oauth.try_fetch_usage_for_account") as fetch, \
             patch.object(switcher, "_write_credentials") as write_live, \
             patch.object(switcher, "_write_account_credentials") as write_backup:
    ...
    def test_on_demand_pass_repairs_reset_parked_exhausted_plan(
        ...
        with patch.object(
            ...
            "altero.oauth.try_fetch_usage_for_account",
    ...
    def test_replan_new_active_pulls_candidate_plan_to_floor(
        ...
        from altero import poll_policy
    ...
    def test_replan_new_active_failure_is_logged_not_raised(
        ...
        ), caplog.at_level(logging.WARNING, logger="altero"):
            switcher._replan_new_active("1", "a@x.com", "")
        assert any(
...
class TestActiveAccountRefresh:
    ...
    def _no_profile_probe(self):
        ...
        with patch(
            "altero.oauth.fetch_oauth_profile", return_value=None
        ):
            yield
    ...
    def test_expired_refreshes_under_locks_and_persists_both_stores(
        ...
        from altero.claude_locks import credentials_lock_dir, oauth_refresh_lock_dir
        ...
        def mock_refresh(credentials, **kw):
            from altero.claude_locks import config_lock_dir
        ...
        with patch.object(switcher, "_read_credentials", return_value=self._EXPIRED), \
             ...
             patch.object(switcher, "_write_credentials") as write_live, \
             patch.object(switcher, "_write_account_credentials") as write_backup, \
             patch("altero.oauth.try_refresh_oauth_credentials",
    ...
    def test_lock_reread_adopts_a_fresher_live_credential(
        ...
        with patch.object(switcher, "_read_credentials", return_value=cc_rotated), \
             ...
             patch.object(switcher, "_write_credentials") as write_live, \
             patch("altero.oauth.try_refresh_oauth_credentials") as mock_refresh, \
             patch("altero.oauth.try_fetch_usage_for_account",
                   side_effect=mock_fetch):
            result = switcher._fetch_active_usage("1", "test@example.com", self._EXPIRED)
    ...
    def test_a_held_consume_lock_defers_the_active_refresh(
        ...
        """
        from altero.locking import FileLock
        ...
        try:
            with patch.object(
                ...
                "altero.oauth.try_refresh_oauth_credentials"
    ...
    def test_filelock_contention_defers_instead_of_raising(
        ...
        """altero's own account FileLock contending (another altero operation in
        ...
        from altero.exceptions import LockError
        ...
        with patch.object(switcher, "_read_credentials", return_value=self._EXPIRED), \
             ...
             patch("altero.switcher.FileLock",
    ...
    def test_unattributed_live_recovers_from_the_slots_backup(
        ...
        """Provenance guard, refined: an expired live credential
    ...
    def test_server_rejected_token_lock_contention_still_surfaces_the_401(
        ...
        from altero.claude_locks import oauth_refresh_lock_dir
        ...
        try:
            with patch.object(
                 ...
                 patch("altero.claude_locks.DEFAULT_TIMEOUT_S", 0.3), \
                 patch("altero.oauth.try_refresh_oauth_credentials") as mock_refresh, \
                 patch("altero.oauth.try_fetch_usage_for_account",
                       return_value=oauth.UsageOutcome(None, error="http-401")):
                result = switcher._fetch_active_usage(
    ...
    def test_non_expired_fetches_without_refreshing(
        ...
        with patch("altero.oauth.try_refresh_oauth_credentials") as mock_refresh, \
             patch("altero.oauth.try_fetch_usage_for_account",
                   return_value=oauth.UsageOutcome({"five_hour": {"pct": 3}})) as mock_fetch:
    ...
    def test_fresh_fetch_resyncs_backup_after_external_rotation(
        ...
        with patch.object(
             ...
             patch.object(switcher, "_write_account_credentials") as write_backup, \
             patch("altero.oauth.fetch_oauth_profile",
    ...
    def _fresh_drift_pass(self, switcher, probe):
        ...
        with patch.object(
             ...
             patch.object(switcher, "_write_account_credentials") as write_backup, \
             patch("altero.oauth.fetch_oauth_profile", **kwargs) as mock_probe, \
             patch("altero.oauth.try_fetch_usage_for_account",
                   return_value=oauth.UsageOutcome({"five_hour": {"pct": 3}})):
            result = switcher._fetch_active_usage(
                "1", "test@example.com", self._REFRESHED
    ...
    def test_fresh_foreign_probe_mismatch_skips_resync_warns_once_and_caches(
        ...
        recorded as this slot's (the credential-poisoning mis-keying shape). The verdict is
        ...
        with caplog.at_level(logging.WARNING, logger="altero"):
    ...
    def test_expiry_after_tolerated_backup_write_failure_consumes_live(
        ...
        with patch.object(switcher, "_read_credentials",
             ...
             patch("altero.oauth.try_fetch_usage_for_account",
                   return_value=oauth.UsageOutcome({"five_hour": {"pct": 5}})), \
             patch("altero.oauth.fetch_oauth_profile") as mock_probe:
    ...
    def test_no_token_returns_no_credentials(
        ...
        from altero.json_output import USAGE_NO_CREDENTIALS

        switcher = self._switcher(sample_sequence_data)
        with patch("altero.oauth.try_fetch_usage_for_account") as mock_fetch:
    ...
    def test_foreign_live_credential_under_the_lock_is_never_consumed(
        ...
        """TOCTOU guard: an `altero switch` completing between the pre-lock
        ...
        write its successor into THIS slot's backup (credential poisoning)."""
        switcher = self._switcher(sample_sequence_data)
        foreign_live = json.dumps({
            "claudeAiOauth": {
...
class TestPerformSwitchPostDisplay:
    ...
    def test_switch_persists_rotated_refresh_token_to_backup(
        ...
        try:
            with patch(
                ...
                "altero.oauth.request_usage_data",
                return_value={
    ...
    def test_switch_uses_live_identity_for_current_backup_slot(
        ...
        live_creds = json.dumps({
            "claudeAiOauth": {
                "accessToken": "second-live-token",
                "refreshToken": "second-live-refresh",
            }
        })
        creds_store = {
            ("3", "first@example.com"): target_creds,
            ("4", "second@example.com"): "old-second-backup",
        }
        configs_store = {
            ("3", "first@example.com"): json.dumps({
                "oauthAccount": {
            ...
            ("4", "second@example.com"): "old-second-config",
        }
        ...
        assert creds_store[("4", "second@example.com")] == live_creds
        assert ("3", "second@example.com") not in creds_store
        assert json.loads(live_state["creds"])["claudeAiOauth"]["accessToken"] == (
            "target-token"
        )
...
class TestSwitchToSelfSlotAndForce:
    """--switch-to onto the active account must not back up the
    ...
    def test_switch_to_current_slot_is_noop_preserving_backup(
        ...
        assert "altero --switch-to 1 --force" in out
...
class TestAccountInfoOrgFields:
    def test_account_info_includes_org_fields(self):
        ...
        from altero.models import AccountInfo
        info = AccountInfo(
...
class TestDeadTokenQuarantine:
    ...
    def _make_dead(self, switcher, num="2", identity=("test@example.com", "")):
        store = switcher._usage_store
        from altero.usage_store import FetchRecord
    ...
    def test_collector_surfaces_relogin_sentinel_and_skips_fetch(self, temp_home):
        ...
        with patch("altero.oauth.try_fetch_usage_for_account") as fetch:
    ...
    def test_drifted_session_profile_keeps_relogin_required(self, temp_home):
        ...
        with patch(
            ...
            "altero.session.session_identity_drifted", return_value=True
        ), patch.object(
            switcher, "_run_usage_fetches"
        ) as run:
            entries = switcher._collect_usage_entries(info)
    ...
    def test_no_session_profile_keeps_relogin_required(self, temp_home):
        ...
        with patch(
            "altero.session.read_session_credentials", return_value=None
        ), patch.object(
            switcher, "_run_usage_fetches"
        ) as run:
            entries = switcher._collect_usage_entries(info)
    ...
    def test_the_collector_hands_the_trust_bound_its_configured_models(
        ...
        from altero.usage_store import SERVE_TTL_S, FetchRecord
...
class TestListAccountsOrgDisplay:
    def test_shows_org_name_and_personal(self, temp_home, mock_credentials_file,
        ...
        switcher = ClaudeAccountSwitcher()
        with patch("altero.oauth.try_fetch_usage_for_account", return_value=oauth.UsageOutcome(None)):
            switcher.list_accounts()
...
class TestPurge:
    ...
    def test_purge_removes_legacy_none_keychain_entry(self, temp_home):
        ...
        with patch("builtins.input", return_value="y"), \
             patch("altero.switcher.macos_keychain") as mock_kc, \
             patch.dict(sys.modules, {"keyring": mock_keyring}):
            switcher.purge()

        # New security service: account + legacy account-None both cleaned.
        mock_kc.delete_password.assert_has_calls([
            call("altero", "account-1-user@example.com"),
            call("altero", "account-None-user@example.com"),
        ])
...
# ---------------------------------------------------------------------------
# tolerate broken slots in switch/switch_to
# ---------------------------------------------------------------------------


class TestSwitchSkipsBrokenSlots:
    """--switch must skip slots whose stored creds or config are
    ...
    def test_switch_to_missing_credentials_actionable_error(self, temp_home: Path):
        ...
        from altero.exceptions import SwitchError
...
class TestClaudeCodeLockCooperation:
    ...
    def test_preheld_cc_lock_fails_cleanly_without_mutation(
        ...
        from altero import claude_locks
        from altero.exceptions import ClaudeCodeLockTimeout
...
class TestMacosKeychainFallback:
    ...
    def test_active_read_retries_transient_keychain_failure(
        ...
        acct = macos_keychain.keychain_account_name()
        block_real_keychain.data[(CLAUDE_CODE_KEYCHAIN_SERVICE, acct)] = "FROM-KC"
        monkeypatch.setattr("altero.credentials._ACTIVE_READ_RETRY_DELAY", 0)
    ...
    def test_keychain_write_refreshes_existing_file(
        ...
        # An already-present shadow file must be rewritten (mtime bumped) so a
        # running Claude Code session invalidates its memoized token and hot-reloads.
        # It is rewritten, never deleted — a file-reading consumer stays valid.
        s = self._macos_switcher()
        cred = get_credentials_path()
        cred.parent.mkdir(parents=True, exist_ok=True)
        ...
        assert s._last_active_credentials_backend == "keychain"
        assert cred.exists()  # never deleted
    ...
    def test_keychain_write_does_not_create_absent_file(
        ...
        # created, so no plaintext credential lands on their disk.
        s = self._macos_switcher()
        cred = get_credentials_path()
...
class TestFormatUsageLines:
    ...
    def test_seven_day_ahead_of_pace_marker(self):
        ...
        # ahead of the ~14% expected at that point.
        from datetime import datetime, timedelta, timezone
    ...
    def test_five_hour_never_shows_pace_marker(self):
        # Pace applies only to weekly windows, never the 5h one.
        from datetime import datetime, timedelta, timezone
...
class TestProvenanceGuard:
    """Fail-open hybrid: the identity oracle is advisory.
    ...
    def test_byte_identical_live_skips_credential_backup(
        ...
        """Nothing rotated since altero's own write → nothing to capture."""
        switcher, creds_store, configs_store = self._setup_two_accounts(
            temp_home, sample_sequence_data,
        )
    ...
    def test_alien_credential_preserved_and_skipped(
        ...
        can't name a slot, so it recommends a plain `altero add`."""
        switcher, creds_store, configs_store = self._setup_two_accounts(
            temp_home, sample_sequence_data,
        )
        creds_store[("1", "test@example.com")] = self._A1_BACKUP
...
class TestSelfSwitchProvenance:
    ...
    def test_diverged_unresolvable_self_switch_noops_silently(
        ...
        try:
            with patch("altero.oauth.fetch_oauth_profile", return_value=None):
                result = switcher.switch_to("1", json_output=True)
        finally:
            for p in patches:
                p.stop()
...
class TestLockstepUsageDetection:
    """Heuristic detector for the different-generation collapse:
...
class TestStashAndRetentionStore:
    ...
    def test_strict_clear_final_belt_fails_closed_on_unreadable_enc(
        ...
        """
        from altero.exceptions import CredentialError
...
class TestSharedOAuthCredentialPreservation:
    ...
    (mcpOAuth et al.) with the destination slot's stale snapshot."""
    ...
    def test_account_bound_and_unknown_siblings_stay_target_owned(
        ...
        # trustedDeviceToken is enrolled per-account, and a field altero does
...
class TestAddAccountAlias:
    ...
    def test_readd_without_alias_preserves_existing(self, temp_home: Path):
        """Re-running `altero add` (refresh-in-place) without --alias must not
...
class TestDisableEnableAccount:
    """`altero disable`/`altero enable`: park a managed account out of automatic
...
class TestDegradedReadProvenance:
    ...
    def test_an_empty_slot_under_pinned_file_mode_reads_as_empty(
        ...
        """
        from altero.credentials import Platform
    ...
    def test_pinned_file_mode_is_not_a_degraded_read(self, temp_home: Path):
        ...
        assert store._read_active_credentials().degraded is False, (
            ...
            "altero stops refreshing the active token for this process"
        )
    ...
    def test_degraded_active_read_never_consumes(
        ...
        from altero.credentials import ActiveCredentials
        from altero.json_output import USAGE_KEYCHAIN_UNAVAILABLE
    ...
    def test_status_path_sets_the_degraded_flag_too(
        ...
        """`altero --status` must arm the same guard the collect pass does.
...
class TestBackupUnreadableDisplay:
    ...
    def test_idle_slot_unreadable_backup_shows_keychain_unavailable(
        ...
        from altero.json_output import (
...
class TestSwitchUnreadableBackup:
    ...
    def test_normal_path_switch_to_unreadable_backup_says_keychain(
        ...
        """The SAME promise on the ordinary `altero switch`.
...
class TestConsumeGate:
    ...
    def test_the_purge_exit_the_fail_closed_message_names_actually_works(
        ...
        operator has a way out, and the message names one: `altero unclaimed`
        ...
        assert listed, "corrupt manifest hid the orphan from `altero unclaimed`"
    ...
    def test_a_corrupt_manifest_with_orphan_entries_fails_closed(
        ...
        (`altero unclaimed --purge`, which still lists orphans by glob), not a
    ...
    def test_a_removed_slot_defers_instead_of_posting_the_snapshot(
        ...
        """
        s = self._switcher(sample_sequence_data)
        # No stored credential for slot 1: `altero remove` landed first.
        posted = []
...
class TestInactiveRefreshRoutesThroughGate:
    ...
    def test_expired_inactive_fetch_uses_gate(
        ...
        with patch("altero.oauth.try_refresh_oauth_credentials",
             ...
             patch.object(s, "_live_session_pids", return_value=[]), \
             patch("altero.oauth.request_usage_data",
...
class TestStrikeUnbindsInCollector:
    ...
    def test_relogin_lifts_after_credential_replaced(
        ...
        from altero.json_output import USAGE_RELOGIN_REQUIRED
        from altero.usage_store import FetchRecord as StoreRecord
        sample_sequence_data["accounts"]["2"] = {
            "email": "b@example.com", "uuid": "u2",
            "organizationUuid": "", "organizationName": "",
        }
...
class TestStoreResolutionParity:
    """M4: when CC resolves its credential store somewhere altero does not
    ...
    def test_securestorage_env_refuses_consume(
        ...
        with patch("altero.oauth.try_refresh_oauth_credentials") as mock_post:
...
class TestConsumeGateLockFailures:
    ...
    def test_lock_failure_before_post_is_transient(
        ...
        from altero.exceptions import LockError as LE
        s = self._switcher(sample_sequence_data)
        s._write_account_credentials("1", "test@example.com", self._OLD)

        from altero.locking import FileLock as real_lock
        ...
        monkeypatch.setattr("altero.switcher.FileLock", FailingLock)
        with patch("altero.oauth.try_refresh_oauth_credentials") as post:
    ...
    def test_lock_failure_after_post_stashes_successor(
        ...
        monkeypatch.setattr("altero.switcher.FileLock", SecondLockFails)
        with patch("altero.oauth.try_refresh_oauth_credentials",
    ...
    def test_a_stashed_successor_is_not_reported_as_freshened(
        ...
        never refresh, and Claude Code logs the account out. The previous code raised
...
class TestPermanentlyUnreadableStashRow:
    ...
    def test_the_kind_carries_its_remedy_and_skips_the_doomed_fetch(self):
        ...
        """
        from altero.oauth import _DETERMINISTIC_REFRESH_ERRORS
        from altero.switcher import ERROR_NOTES

        assert "altero unclaimed" in ERROR_NOTES["stash-unreadable"]
...
class TestGateUltraReviewFixes:
    ...
    def test_no_store_read_happens_before_the_consume_lock(
        ...
        from altero.locking import FileLock as real_filelock
        ...
        monkeypatch.setattr("altero.switcher.FileLock", WatchedLock)
    ...
    def test_contention_reports_its_own_kind_not_transient(
        ...
        affect. On a machine where the collector and a manual `altero switch`
        ...
        monkeypatch.setattr("altero.switcher.FileLock", ConsumeLockBusy)
        out = s.consume_backup_grant("1", "test@example.com", self._OLD)
    ...
    def test_stale_marked_profile_never_supersedes_backup(
        ...
        expiresAt is newer."""
        from altero.session import STALE_MARKER, session_dir_for
        s = self._switcher(sample_sequence_data)
    ...
    def test_a_foreign_profile_never_supersedes_the_backup(
        ...
        """Credential poisoning through the gate's resync.
    ...
    def test_a_denied_session_dir_still_leaves_the_stale_marker(
        ...
        from altero.session import is_session_stale
        ...
        if denied == "session_dir":
            assert is_session_stale(sess), (
                ...
                "local reuse check passes and `altero run` launches claude on "
...
class TestActiveSlotStrikeParity:
    ...
    def test_active_strike_bound_to_backup_not_healed_by_live_fp(
        ...
        from altero.usage_store import FetchRecord as FR
    ...
    def test_degraded_read_never_feeds_resync_write(
        ...
        from altero.oauth import UsageOutcome
        sample_sequence_data["accounts"]["2"]["email"] = "b@example.com"
        s = ClaudeAccountSwitcher()
        s._setup_directories()
        s._write_json(s.sequence_file, sample_sequence_data)
...
class TestUnreadableBackupIsNotAbsent:
    ...
    def test_unreadable_backup_is_not_a_dead_slot_for_import(
        ...
        """``altero import`` must not overwrite a healthy slot it cannot read.
