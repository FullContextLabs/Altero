...
from altero.autoswitch import NoSwitchEvent, SwitchEvent
from altero.json_output import USAGE_API_KEY, USAGE_TOKEN_EXPIRED
from altero.models import AccountSnapshot, AccountsSnapshot
from altero.switcher import ClaudeAccountSwitcher
from altero.tui import data as tui_data
from altero.usage_store import UsageEntry
...
async def menu_select(pilot, action_id: str) -> None:
    ...
    from altero.tui.widgets import MenuItem
...
class TestFormatting:
    ...
    def test_sentinel_labels_match_altero_list(self):
        # The TUI must describe sentinel states with the exact wording `altero
    ...
    def test_sentinel_card_shows_last_seen_like_altero_list(self):
        # A sentinel is a live overlay — the entry can still carry the last
        # good measurement, and `altero list` prints it as a "last seen" line.
        # The card must too (except for API-key accounts, which have no quota).
        from altero.tui.widgets import account_card_text
    ...
    def test_account_card_uses_light_palette_when_passed(self):
        from altero.tui.theme import ACCENT_LIGHT, ALTERO_LIGHT, Palette
        from altero.tui.widgets import account_card_text

        acc = make_account(1, active=True, entry=make_entry(pct5=95.0))
        text = account_card_text(acc, 100, palette=Palette.from_theme(ALTERO_LIGHT))
...
class TestSnapshotSource:
    ...
    def test_every_pass_is_store_governed(self, tmp_path):
        ...
        # reservation), so every take is the same on-demand pass `altero list`
...
class TestUsageRows:
    ...
    def test_absent_window_produces_no_row(self):
        from altero.tui.widgets import usage_rows
...
class TestMiniAccountText:
    def test_seven_day_ahead_of_pace_marker(self):
        from altero.tui.widgets import mini_account_text
...
class TestDashboard:
    ...
    async def test_s_opens_switch_screen_and_enter_switches(self, tmp_path):
        ...
        async with app.run_test(size=(100, 32)) as pilot:
            ...
            from altero.tui.dashboard import DashboardScreen, SwitchScreen
            from altero.tui.widgets import AccountItem
    ...
    async def test_palette_is_disabled(self, tmp_path):
        ...
        assert AlteroApp.ENABLE_COMMAND_PALETTE is False
...
class TestWatchScreen:
    ...
    async def test_w_opens_monitor_without_cursor(self, tmp_path):
        ...
        async with app.run_test(size=(100, 40)) as pilot:
            ...
            from altero.tui.dashboard import WatchScreen
            from altero.tui.widgets import AccountItem
    ...
    async def test_escape_disarms_then_leaves(self, tmp_path):
        ...
        async with app.run_test(size=(100, 40)) as pilot:
            ...
            from altero.tui.dashboard import DashboardScreen, WatchScreen
    ...
    async def test_app_start_watch_stacks_over_dashboard(self, tmp_path):
        ...
        app = AlteroApp(self._fake(tmp_path), start="watch")
        async with app.run_test(size=(100, 40)) as pilot:
            await settle(pilot)
            from altero.tui.dashboard import DashboardScreen, WatchScreen
...
def fake_engine(monkeypatch):
    ...
    monkeypatch.setattr(
        "altero.tui.autoview.AutoSwitchEngine", _FakeEngine
    )
    return _FakeEngine
...
class TestAutoScreen:
    ...
    async def test_opens_in_dry_run_and_store_only(self, tmp_path, fake_engine):
        ...
        async with app.run_test(size=(100, 40)) as pilot:
            ...
            from altero.tui.autoview import AutoScreen
    ...
    async def test_defers_to_a_running_backend_service(
        ...
        monkeypatch.setattr(
            "altero.tui.autoview.launch_agent.engine_owner",
            lambda _dir: (launch_agent.ENGINE_BACKEND, "pid 4242 (/tmp/altero auto)"),
        )
        fake = FakeSwitcher(
            [make_account(1, active=True), make_account(2)], tmp_path
        )
    ...
    async def test_a_foreign_engine_is_neither_ours_nor_the_backends(
        ...
        """The bug this replaces: a hand-run `altero auto` held the lock, the
        ...
        monkeypatch.setattr(
            ...
            lambda _dir: (
                ...
                "pid 4242 (/usr/local/bin/altero auto)",
            ),
        )
        fake = FakeSwitcher(
            [make_account(1, active=True), make_account(2)], tmp_path
    ...
    async def test_a_just_started_backend_is_the_owner_before_it_locks(
        ...
        app = AlteroApp(fake, backend_managed=True)
        async with app.run_test(size=(100, 40)) as pilot:
            await self._open(pilot)
            from textual.widgets import Static
    ...
    async def test_falls_back_to_hosting_when_the_backend_failed(
        ...
        app = AlteroApp(fake, backend_managed=False, backend_error="exit 5")
        async with app.run_test(size=(100, 40)) as pilot:
            await self._open(pilot)
            from textual.widgets import RichLog
    ...
    async def test_back_stops_engine_and_restores_fetching(
        ...
        async with app.run_test(size=(100, 40)) as pilot:
            ...
            await pilot.press("escape")
            await settle(pilot)
            from altero.tui.dashboard import DashboardScreen
...
class TestEventText:
    def test_switch_event_styling_and_content(self):
        ...
        from altero.tui.autoview import event_text
    ...
    def test_event_text_uses_light_accent_for_switch(self):
        ...
        text = event_text(event, palette=Palette.from_theme(ALTERO_LIGHT))
...
class TestBareInvocation:
    def test_bare_tty_launches_tui(self, monkeypatch, temp_home):
        import altero.cli as cli
        import altero.tui as tui
        ...
        monkeypatch.setattr(sys, "argv", ["altero"])
    ...
    def test_altero_watch_opens_tui_on_watch_page(self, monkeypatch, temp_home):
        import altero.cli as cli
        import altero.tui as tui
        ...
        monkeypatch.setattr(sys, "argv", ["altero", "watch"])
        monkeypatch.setattr(tui, "run", fake_run)
        with pytest.raises(SystemExit) as excinfo:
            cli.main()
        assert excinfo.value.code == 0
...
class TestThemeWiring:
    ...
    async def test_auto_setting_no_detection_falls_back_to_dark(self, tmp_path):
        ...
        fake = FakeSwitcher([make_account("1", active=True)], tmp_path)
        from altero.tui.app import AlteroApp
        app = AlteroApp(fake, detected=None)
        async with app.run_test() as pilot:
            await settle(pilot)
            assert app.theme == "altero-dark"
    ...
    async def test_toggle_cycles_dark_light_auto(self, tmp_path):
        ...
        async with app.run_test() as pilot:
            await settle(pilot)
            assert app.theme == "altero-dark"          # setting dark
            app.action_toggle_theme(); await pilot.pause()
            assert app.theme == "altero-light"          # → light
            app.action_toggle_theme(); await pilot.pause()
            assert app.theme == "altero-light"          # → auto, detected=light
            assert json.loads((tmp_path / "settings.json").read_text())["ui"]["theme"] == "auto"
            app.action_toggle_theme(); await pilot.pause()
            assert app.theme == "altero-dark"           # → back to dark
