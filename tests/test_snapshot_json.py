"""Tests for ``altero snapshot`` — the one-shot display-state JSON document.
...
from altero.json_output import SCHEMA_VERSION, USAGE_API_KEY, USAGE_TOKEN_EXPIRED
from altero.models import AccountSnapshot, AccountsSnapshot
from altero.snapshot_json import snapshot_payload, take_snapshot, write_snapshot
...
def test_write_snapshot_publishes_by_rename(tmp_path: Path):
    ...
    with patch("altero.fsutil.os.replace", spy):
        write_snapshot(out, {"schemaVersion": SCHEMA_VERSION})
...
class TestWriteSnapshotThroughSymlink:
    """Same rule as ``settings.atomic_write_json``: a rename
    ...
    def test_the_temp_file_goes_beside_the_resolved_target(self, tmp_path: Path):
        ...
        with patch("altero.snapshot_json.tempfile.mkstemp", spy):
            write_snapshot(link, {"schemaVersion": SCHEMA_VERSION})
...
def test_write_snapshot_leaves_no_temp_file_on_failure(tmp_path: Path):
    out = tmp_path / "snapshot.json"
    with patch("altero.fsutil.os.replace", side_effect=OSError("boom")):
...
class _FakeSwitcher:
    ...
    def __init__(self, snap: AccountsSnapshot, backup_dir: Path | None = None):
        ...
        self.backup_dir = backup_dir or Path("/nonexistent-altero-backup")
...
def _run(argv: list[str]) -> None:
    with patch.object(sys, "argv", ["altero", *argv]):
        cli.main()
...
def test_snapshot_never_probes_the_terminal_background():
    from altero.appearance import cli_should_probe
...
# What the engine passes as ``altero_command`` (``launch_agent.resolve_program``).
_GOLDEN_ALTERO_COMMAND = ["/Users/dev/.local/bin/altero"]
...
def _golden_autoswitch(backup_dir: Path) -> dict:
    from altero.json_output import iso_timestamp
    from altero.usage_history import UsageHistory
...
def test_snapshot_matches_the_committed_golden_fixture(frozen_clock, tmp_path):
    from altero.snapshot_json import history_for
    ...
    assert payload == json.loads(_GOLDEN_PATH.read_text()), (
        "The `altero snapshot` JSON schema changed: this payload no longer "
