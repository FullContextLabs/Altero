...
from altero.state_watch import ACCOUNTS, ACTIVE, SETTINGS, StateWatcher
...
def state(tmp_path: Path):
    """A config file and a backup root holding the two altero state files."""
...
def test_mappings_are_deliberately_not_watched(state):
    """`altero map` writes mappings.json, but no surface renders directory
