...
/// `~/.altero/widget-requests/`. The backend applies a fresh one
/// (under 60s old) through the same path as `altero switch N` and republishes
...
enum SwitchEligibility: Equatable, Sendable {
    ...
    case active
    /// No stored credentials/config backup: `altero switch` would refuse it.
    case notSwitchable
...
extension Account {
    ...
    /// (`apply_switch_request` in `src/altero/widget_requests.py`): the
    /// slot must hold a usable backup (`switchable`) and must not be
    /// disabled. `altero switch N` does take a disabled slot, but the backend
