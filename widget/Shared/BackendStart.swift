...
// `altero service start`). Pure Foundation, so the test target exercises it.
//
...
// The host app records it instead: before running altero it drops
...
enum BackendStart {
    ...
    static let url = URL(string: "altero://start-backend")!

    static func isStartURL(_ url: URL) -> Bool {
        url.scheme?.lowercased() == "altero" && url.host?.lowercased() == "start-backend"
    }
    ...
    /// Where altero may be installed when the snapshot does not name it
    /// (snapshots written before `alteroCommand` existed).
    static func fallbackPaths(home: String) -> [String] {
        ["\(home)/.local/bin/altero", "/opt/homebrew/bin/altero", "/usr/local/bin/altero"]
    }

    /// Whether the host app may exec what the snapshot names.
    ///
    /// The snapshot is the user's own 0600 file, so a doctored `alteroCommand`
    ...
    /// absolute `altero`, or an interpreter running `-m altero` -- and
    ...
    static func isAcceptableCommand(_ command: [String], mode: (String) -> Int?) -> Bool {
        ...
        guard (executable as NSString).lastPathComponent == "altero" || arguments == ["-m", "altero"] else {
            return false
        }
