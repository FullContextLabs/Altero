...
/// all live in the `altero` CLI/TUI and the menu bar.
///
/// Three ways in:
///
/// - `--placed-widgets`: answers one question for the backend and exits
///   without ever creating an NSApplication.
/// - `altero://start-backend` (the widget's Start control): runs
...
/// request directory and mirrored to `os_log` (subsystem `com.fullcontextlabs.altero`).
/// The app has no window on the Start path, so without this a start that goes
/// wrong leaves nothing behind to read.
enum HostLog {
    private static let logger = Logger(subsystem: "com.fullcontextlabs.altero", category: "host")
...
enum BackendStarter {
    ...
    private static func execute() -> Result<Void, Failure> {
        ...
        // Output to a file, not a pipe: if altero leaves a child holding the
        // descriptor, reading a pipe to its end would never return.
        let log = FileManager.default.temporaryDirectory.appending(path: "altero-start-\(UUID().uuidString).log")
        FileManager.default.createFile(atPath: log.path, contents: nil)
        defer { try? FileManager.default.removeItem(at: log) }
        guard let output = try? FileHandle(forWritingTo: log) else {
            return .failure(Failure(message: "Could not create a log file for altero's output."))
        }
        ...
        // shell's; altero and whatever it runs expect the usual places.
...
/// `Altero --placed-widgets` prints `{"count": N}` -- how many Altero
...
enum PlacedWidgets {
    ...
    private static func fail(_ message: String) -> Never {
        FileHandle.standardError.write(Data("Altero --placed-widgets: \(message)\n".utf8))
        exit(1)
    }
}
