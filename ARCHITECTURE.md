...
`altero` does its work in one place: a headless backend that runs as a launchd
...
`altero auto` started one. Run two surfaces and two engines make independent
...
| someone ran `altero auto` in a terminal | yes |
...
Surfaces do not send it requests, and most of what `altero` does never touches
it. `altero switch`, `altero add`, `altero remove`, `altero map`, `altero alias` all
...
cannot run `altero switch` itself, so the backend runs it on the widget's
...
`~/.altero/snapshot.json` is read by a **separately distributed**
...
- `accounts[].usage.fiveHour.history` — `[{t, pct}]`, 24h, oldest first, at
  ...
  measurement is in the store; it survives backend restarts. `altero snapshot`
  reads it too.
- top-level `autoswitch` — `enabled`, the effective `threshold`,
  ...
  engine-published file carries it; the one-shot `altero snapshot` omits it.
...
- top-level `alteroCommand` — the argv prefix that runs this altero, exactly
  what `launch_agent.resolve_program()` returns and the backend plist runs
  (e.g. `["/Users/me/.local/bin/altero"]`, or `[python, "-m", "altero"]`
  without a console script). The widget's host app runs `alteroCommand +
  ["service", "start"]` for its "Start backend" button. Engine file only,
  like `autoswitch`.

## How surfaces stay current

A change can originate anywhere: you type `altero switch` in a terminal, you add
...
app or extension — a Python CLI has no way to call it. So nothing `altero` does
...
Turning the policy off (any surface's toggle, or `altero config set
autoswitch.enabled false`) keeps the backend running for its measurements.
Manual switching (`altero switch`, `altero run`, the menu bar's account list)
...
`altero` is the only entry point. Nobody installs the backend; the surfaces
start it.

```
altero                     TUI (also `altero tui`, `altero watch`)
altero menubar             install + start the menu bar agent, return the prompt
altero service status      is the backend running, and which build is launchd holding
altero service logs        tail the event stream
altero service start       ensure the backend is running, without a surface
```

The widget has no `altero` subcommand: it is built and installed by
`./widget/build-widget` (see "Not built", below).

`altero auto` remains the foreground / `--once` engine for cron and debugging.
It is no longer the thing you install. (`altero auto --install-service` still
...
The menu bar agent runs `altero menubar --foreground` (hidden flag); the backend
agent runs `altero auto --json --backend`.
...
4. **A placed widget counts as a viewer.** It cannot hold a surface lock —
   ...
   once to `com.fullcontextlabs.altero.auto.err`. The answer is cached for 5 minutes, so
...
covers `altero menubar`, which ensures the backend and exits before the menu
bar app registers (the app ensures the backend again itself, which is also
what brings it back at login).

A backend that finds the engine lock held (a hand-run `altero auto`) logs it
...
`altero auto` refused the same way still exits 1.

#### The backend trims its own logs

The plist points `StandardOutPath`/`StandardErrorPath` at
`~/Library/Logs/com.fullcontextlabs.altero.auto.{log,err}`, and the stdout one is the event
...
switches. `tail -f`, what `altero service logs -f` runs, reports the
...
`epochMillis`) through `settings.set_setting`, the writer behind `altero
...
`com.fullcontextlabs.altero.auto.err` and followed by `engine.wake()`: the tick re-reads
...
on a fresh switcher, exactly what `altero switch N` runs, so validation and
...
is refused, where `altero switch N` accepts it — a tap on a dimmed row is far
...
It gets the same engine treatment as `altero switch`, which is **the
...
mid-call), so the current session keeps running. `altero menubar
--uninstall-service` is the stop-now path: bootout + delete, from outside the
app.

Both labels (`com.fullcontextlabs.altero.auto`, `com.fullcontextlabs.altero.menubar`) can be overwritten by
either a dev checkout or a global `uv tool install`; `altero service status`
...
- **App Groups don't work here.** The container is reachable by a signed
  reader, but the *writer* is the user's own `altero`, installed from PyPI. A
...
**It cannot change any altero state.** Not switching, not adding or removing an
...
read-only exception can do none of it. It also cannot exec `altero`.
...
container. So an intent can change what the widget *shows*, just not what altero
*does*:
...
- `altero snapshot` and the snapshot schema, with the golden fixture and drift
  tests on both sides
...
- `mappings.json` is not watched — no surface renders directory mappings, so
  an `altero map` has nothing to repaint. One line in `state_watch` the day one does.
- a surface cannot read a *foreign* engine's events: only the backend has a
  log path anyone else knows. A hand-run `altero auto` shows as EXTERNAL, and
  its decisions stay in its own terminal.
- `altero widget install` — no install path exists; the widget is built by hand
- cross-process `wake()`
...
- five paths against the real host app and widget, each unit-tested with the
  ...
  widget row, an **upgrade-triggered reinstall** (`ALTERO_VERSION` differing),
...
`altero` or `altero menubar`, and then the TUI, the menu bar and the widget all
